...
class groupModel extends validationClass
{
    ...
    function execute ()
    {
         ...
         */
        if (isset($_POST['Id'])) {
            $this->setGroupId($this->strict($_POST['groupId'], 'numeric'), 0, 
            'single');
        }
        if (isset($_POST['groupSequence'])) {
            $this->setGroupSequence(
            $this->strict($_POST['groupSequence'], 'memo'));
        }
        ...
        for ($i = 0; $i < $this->getTotal(); $i ++) {
            $this->setGroupId($this->strict($_GET['groupId'][$i], 'numeric'), 
            $i, 'array');
    ...
    public function setGroupId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:setGroupId ?"));
                exit();
            }
    }
    /**
    ...
    public function getGroupId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:getGroupId ?"));
                exit();
            }
    }
    /**
