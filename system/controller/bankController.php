...
class BankClass extends ConfigClass {
	...
	public function create() {
		...
		$end = microtime(true);
		$time = $end - $start;
		echo json_encode(array("success" => true, "message" =>  $this->systemString->getCreateMessage(), "bankId" => $bankId));
		exit();
	}
	...
	public function read() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT		BANK.BANKID   		 		AS 	\"bankId\",
						...
						BANK.BANKSWIFTCODE 			AS  \"bankSwiftCode\",
						BANK.BANKSWIFTCODECITY 		AS  \"bankSwiftCodeCity\",
						BANK.BANKSWIFTCODEBRANCH 	AS  \"bankSwiftCodeBranch\",
						BANK.BANKMEPSCODE 			AS  \"bankMepsCode\",
						BANK.BANKDESC				AS 	\"bankDesc\",							
				...
				$sql = "
							...
							(
								SELECT 		[bank].[bankId],
											...
											[bank].[executeTime],
											[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [bankId]) AS 'RowNumber'
								FROM 	[bank]
								JOIN		[staff]
								ON		[bank].[executeBy] = [staff].[staffId]
				...
				$sql = "
						...
						FROM (
								SELECT	BANK.BANKID   				AS 	\"bankId\",
										...
										BANK.BANKDESC				AS 	\"bankDesc\",
										BANK.BANKSWIFTCODE 			AS  \"bankSwiftCode\",
										BANK.BANKSWIFTCODECITY 		AS  \"bankSwiftCity\",
										BANK.BANKSWIFTCODEBRANCH 	AS  \"bankSwiftBranch\",
										BANK.BANKMEPSCODE 			AS  \"bankMepsCode\",										
										BANK.ISDEFAULT    			AS	\"isDefault\",
										BANK.ISNEW		  			AS	\"isNew\",
										BANK.ISDRAFT	 			AS	\"isDraft\",
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['bankDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
...
if (isset($_GET ['method'])) {
	 ...
	 */
	if (isset($_GET ['bankDesc'])) {
		if (strlen($_GET ['bankDesc']) > 0) {
