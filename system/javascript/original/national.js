Ext.onReady(function() {
    ...
    var staffByProxy = new Ext.data.HttpProxy({
        url: '../controller/nationalController.php?',
        method: 'GET',
    ...
    var nationalProxy = new Ext.data.HttpProxy({
        url: '../controller/nationalController.php',
        method: 'POST',
        success: function(response, options) {
            jsonResponse = Ext.decode(response.responseText);
    ...
    var nationalReader = new Ext.data.JsonReader({
        totalProperty: 'total',
        successProperty: 'success',
        messageProperty: 'message',
        idProperty: 'nationalId'
    });
    var nationalStore = new Ext.data.JsonStore({
        proxy: nationalProxy,
        reader: nationalReader,
        autoLoad: true,
        autoDestroy: true,
        pruneModifiedRecords: true,
        baseParams: {
    ...
    var nationalFilters = new Ext.ux.grid.GridFilters({
        encode: false,
        local: false,
        filters: [ {
            type: 'list',
    ...
    var nationalColumnModel = [new Ext.grid.RowNumberer(), {
        dataIndex: 'nationalCode',
        header: nationalCodeLabel,
        sortable: true,
        hidden: false,
        width : 200
    },{
        dataIndex: 'nationalDesc',
        header: nationalDescLabel,
        sortable: true,
        hidden: false,
        width : 200
    },
    ...
    var nationalFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
    var nationalGrid = new Ext.grid.GridPanel({
        name: 'nationalGrid',
        id: 'nationalGrid',
        border: false,
        store: nationalStore,
        autoHeight: false,
        height: 400,
        columns: nationalColumnModel,
        plugins: [nationalFilters],
        selModel: new Ext.grid.RowSelectionModel({
            singleSelect: true
        }),
        viewConfig: {
        ...
        listeners: {
            'rowclick': function(object, rowIndex, e) {
                var record = nationalStore.getAt(rowIndex);
                ...
                formPanel.form.load({
                    ...
                    params: {
                        method: 'read',
                        mode: 'update',
                        nationalId: record.data.nationalId,
                        leafId: leafId,
                        isAdmin: isAdmin
                    },
                    success: function(form, action) {
                        Ext.getCmp('nationalDescTemp').setValue(record.data.nationalDesc);
        ...
        tbar: {
            items: [{
                ...
                listeners: {
                    'click': function(button,e) {
                        nationalStore.each(function(record,fn,scope) {
                            for (var access in nationalFlagArray) {
                                record.set(nationalFlagArray[access], true);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        nationalStore.each(function(record,fn,scope) {
                            for (var access in nationalFlagArray) {
                                record.set(nationalFlagArray[access], false);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        var url = '../controller/nationalController.php?';
                        var sub_url = '';
                        var modified = nationalStore.getModifiedRecords();
                        for (var i = 0; i < modified.length; i++) {
                            var dataChanges = modified[i].getChanges();
                            sub_url = sub_url + '&nationalId[]=' + modified[i].get('nationalId');
                            if (isAdmin == 1) {
    ...
    var gridPanel = new Ext.Panel({
     ...
     // form entry
    var nationalCode = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: nationalCodeLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'nationalCode',
        name: 'nationalCode',
        id: 'nationalCode',
        allowBlank: false,
    ...
    var nationalDesc = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: nationalDescLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'nationalDesc',
        name: 'nationalDesc',
        id: 'nationalDesc',
        allowBlank: false,
        blankText: blankTextLabel,
    ...
    var nationalCodeTemp = new Ext.form.Hidden({
        name: 'nationalCodeTemp',
        id: 'nationalCodeTemp'
    });
    var checkDuplicateCode = new Ext.Button ({
    	...
    	listeners: {
            'click': function(button,e) {
                if (Ext.getCmp('nationalCode').getValue().length > 0) {
                    Ext.Ajax.request({
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                if (jsonResponse.total > 0) {
                                    if (Ext.getCmp('nationalCodeTemp').getValue() != Ext.getCmp('nationalCode').getValue()) {
                                        duplicate = 1;
                                        duplicateMessageLabel = duplicateMessageLabel + Ext.util.Format.uppercase(Ext.getCmp('nationalCode').getValue()) + ':' + +Ext.util.Format.uppercase(jsonResponse.religionDesc);
    ...
    var nationalId = new Ext.form.Hidden({
        name: 'nationalId',
        id: 'nationalId'
    }); // end form entry
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            ...
            items: [nationalId ,{
				...
				items:[nationalCode,checkDuplicateCode]
} ,nationalDesc,nationalCodeTemp]
        },
        {
            xtype: 'fieldset',
        ...
        },
        {
            ...
            iconCls: 'new',
            handler: function() {
                var id = Ext.getCmp('nationalId').getValue();
                var method = 'create';
                formPanel.getForm().submit({
                    ...
                    success: function(form, action) {
                        if (action.result.success == true) {
                            ...
                            Ext.getCmp('nationalId').setValue(action.result.nationalId);
                        } else {
                            Ext.MessageBox.alert(systemErrorLabel, action.result.message);
                        }
                    },
                    ...
                    fn: function(response) {
                        if ('yes' == response) {
                            Ext.Ajax.request({
                                ...
                                params: {
                                    method: 'delete',
                                    nationalId: Ext.getCmp('nationalId').getValue(),
                                    leafId: leafId,
                                    isAdmin: isAdmin
                                },
                                success: function(response, options) {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        nationalId: Ext.getCmp('firstRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('firstRecord').getValue() >= 1) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            nationalId: Ext.getCmp('previousRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            nationalId: Ext.getCmp('nextRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
        ...
        },
        {
            ...
            handler: function() {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        nationalId: Ext.getCmp('lastRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
                ...
                if (Ext.getCmp('nationalId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        url: '../controller/nationalController.php',
                        method: 'POST',
                        waitTitle: systemLabel,
