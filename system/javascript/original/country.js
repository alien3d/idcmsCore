Ext.onReady(function() {
    ...
    var countryColumnModel = [new Ext.grid.RowNumberer(), {
    ...
    var countryCodeTemp = new Ext.form.Hidden({
        name: 'countryCodeTemp',
        id: 'countryCodeTemp'
    });
    var checkDuplicateCode = new Ext.Button ({
    	...
    	listeners: {
            'click': function(button,e) {
                if (Ext.getCmp('countryCode').getValue().length > 0) {
                    Ext.Ajax.request({
                        ...
                        params: {
                            method: 'duplicate',
                            leafId: leafId,
                            countryCode	: Ext.getCmp('countryCode').getValue()
                        },
                        success: function(response, options) {
                            jsonResponse = Ext.decode(response.responseText);
                            if (jsonResponse.success == true) {
                                if (jsonResponse.total > 0) {
                                    if (Ext.getCmp('countryCodeTemp').getValue() != Ext.getCmp('countryCode').getValue()) {
                                        duplicate = 1;
                                        duplicateMessageLabel = duplicateMessageLabel + Ext.util.Format.uppercase(Ext.getCmp('countryCode').getValue()) + ':' + +Ext.util.Format.uppercase(jsonResponse.religionDesc);
                                        Ext.MessageBox.alert(systemErrorLabel, duplicateMessageLabel);
                                        Ext.getCmp('countryCode').setValue('');
                                    } else {
                                    	Ext.MessageBox.alert(systemErrorLabel, jsonResponse.message);
                                    }
                                }
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            ...
            items: [countryId ,{
				...
				items:[countryCode,checkDuplicateCode]
} ,countryDesc,countryCodeTemp]
        },
        {
            xtype: 'fieldset',
