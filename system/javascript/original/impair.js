Ext.onReady(function() {
    ...
    var staffByProxy = new Ext.data.HttpProxy({
        url: '../controller/impairController.php?',
        method: 'GET',
    ...
    var impairProxy = new Ext.data.HttpProxy({
        url: '../controller/impairController.php',
        method: 'POST',
        success: function(response, options) {
            jsonResponse = Ext.decode(response.responseText);
    ...
    var impairReader = new Ext.data.JsonReader({
        totalProperty: 'total',
        successProperty: 'success',
        messageProperty: 'message',
        idProperty: 'impairId'
    });
    var impairStore = new Ext.data.JsonStore({
        proxy: impairProxy,
        reader: impairReader,
        autoLoad: true,
        autoDestroy: true,
    ...
    var impairFilters = new Ext.ux.grid.GridFilters({
        encode: false,
        local: false,
        filters: [ {
            type: 'list',
    ...
    var impairFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
    var impairGrid = new Ext.grid.GridPanel({
        name: 'impairGrid',
        id: 'impairGrid',
        border: false,
        store: impairStore,
        autoHeight: false,
        height: 400,
        columns: impairColumnModel,
        plugins: [impairFilters],
        selModel: new Ext.grid.RowSelectionModel({
            singleSelect: true
        }),
        viewConfig: {
        ...
        listeners: {
            'rowclick': function(object, rowIndex, e) {
                var record = impairStore.getAt(rowIndex);
                ...
                formPanel.form.load({
                    ...
                    params: {
                        method: 'read',
                        mode: 'update',
                        impairId: record.data.impairId,
                        leafId: leafId,
                        isAdmin: isAdmin
                    },
                    success: function(form, action) {
                        Ext.getCmp('impairDescTemp').setValue(record.data.impairDesc);
        ...
        tbar: {
            items: [{
                ...
                listeners: {
                    'click': function(button,e) {
                        impairStore.each(function(record,fn,scope) {
                            for (var access in impairFlagArray) {
                                record.set(impairFlagArray[access], true);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        impairStore.each(function(record,fn,scope) {
                            for (var access in impairFlagArray) {
                                record.set(impairFlagArray[access], false);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        var url = '../controller/impairController.php?';
                        var sub_url = '';
                        var modified = impairStore.getModifiedRecords();
                        for (var i = 0; i < modified.length; i++) {
                            var dataChanges = modified[i].getChanges();
                            sub_url = sub_url + '&impairId[]=' + modified[i].get('impairId');
                            if (isAdmin == 1) {
    ...
    var gridPanel = new Ext.Panel({
     ...
     // form entry
    var impairCode = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: familyCodeLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'impairCode',
        name: 'impairCode',
        id: 'impairCode',
        allowBlank: false,
    ...
    var impairDesc = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: impairDescLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'impairDesc',
        name: 'impairDesc',
        id: 'impairDesc',
        allowBlank: false,
        blankText: blankTextLabel,
    ...
    var impairCodeTemp = new Ext.form.Hidden({
        name: 'impairCodeTemp',
        id: 'impairCodeTemp'
    });
    var checkDuplicateCode = new Ext.Button ({
    	...
    	listeners: {
            'click': function(button,e) {
                if (Ext.getCmp('impairCode').getValue().length > 0) {
                    Ext.Ajax.request({
                        ...
                        params: {
                            method: 'duplicate',
                            leafId: leafId,
                            impairCode	: Ext.getCmp('impairCode').getValue()
                        },
                        success: function(response, options) {
                            jsonResponse = Ext.decode(response.responseText);
                            if (jsonResponse.success == true) {
                                if (jsonResponse.total > 0) {
                                    if (Ext.getCmp('impairCodeTemp').getValue() != Ext.getCmp('impairCode').getValue()) {
                                        duplicate = 1;
                                        duplicateMessageLabel = duplicateMessageLabel + Ext.util.Format.uppercase(Ext.getCmp('impairCode').getValue()) + ':' + +Ext.util.Format.uppercase(jsonResponse.religionDesc);
                                        Ext.MessageBox.alert(systemErrorLabel, duplicateMessageLabel);
                                        Ext.getCmp('impairCode').setValue('');
                                    } else {
                                    	Ext.MessageBox.alert(systemErrorLabel, jsonResponse.message);
                                    }
                                }
    ...
    var impairId = new Ext.form.Hidden({
        name: 'impairId',
        id: 'impairId'
    }); // end form entry
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            ...
            items: [impairId,{
				...
				items:[impairCode,checkDuplicateCode]
}, impairDesc,impairCodeTemp]
        },
        {
            xtype: 'fieldset',
        ...
        },
        {
            ...
            iconCls: 'new',
            handler: function() {
                var id = Ext.getCmp('impairId').getValue();
                var method = 'create';
                formPanel.getForm().submit({
                    ...
                    success: function(form, action) {
                        if (action.result.success == true) {
                            ...
                            Ext.getCmp('impairId').setValue(action.result.impairId);
                        } else {
                            Ext.MessageBox.alert(systemErrorLabel, action.result.message);
                        }
                    },
                    ...
                    fn: function(response) {
                        if ('yes' == response) {
                            Ext.Ajax.request({
                                ...
                                params: {
                                    method: 'delete',
                                    impairId: Ext.getCmp('impairId').getValue(),
                                    leafId: leafId,
                                    isAdmin: isAdmin
                                },
                                success: function(response, options) {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        impairId: Ext.getCmp('firstRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('firstRecord').getValue() >= 1) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            impairId: Ext.getCmp('previousRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            impairId: Ext.getCmp('nextRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
        ...
        },
        {
            ...
            handler: function() {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        impairId: Ext.getCmp('lastRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
                ...
                if (Ext.getCmp('impairId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        url: '../controller/impairController.php',
                        method: 'POST',
                        waitTitle: systemLabel,
