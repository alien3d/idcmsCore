Ext.onReady(function() {
    ...
    var staffByProxy = new Ext.data.HttpProxy({
        url: '../controller/genderController.php?',
        method: 'GET',
    ...
    var genderFilters = new Ext.ux.grid.GridFilters({
        encode: false,
        local: false,
        filters: [ {
            type: 'list',
    ...
    var genderColumnModel = [new Ext.grid.RowNumberer(), {
    	 dataIndex: 'stateId',
    ...
    var genderFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
    var genderGrid = new Ext.grid.GridPanel({
        name: 'genderGrid',
        id: 'genderGrid',
        border: false,
        store: genderStore,
        autoHeight: false,
        height: 400,
        columns: genderColumnModel,
        plugins: [genderFilters],
        selModel: new Ext.grid.RowSelectionModel({
            singleSelect: true
        }),
        viewConfig: {
        ...
        listeners: {
            'rowclick': function(object, rowIndex, e) {
                var record = genderStore.getAt(rowIndex);
                ...
                formPanel.form.load({
                    ...
                    params: {
                        method: 'read',
                        mode: 'update',
                        genderId: record.data.genderId,
                        leafId: leafId,
                        isAdmin: isAdmin
                    },
                    success: function(form, action) {
                        Ext.getCmp('genderDescTemp').setValue(record.data.genderDesc);
        ...
        tbar: {
            items: [{
                ...
                listeners: {
                    'click': function(button,e) {
                        genderStore.each(function(record,fn,scope) {
                            for (var access in genderFlagArray) {
                                record.set(genderFlagArray[access], true);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        genderStore.each(function(record,fn,scope) {
                            for (var access in genderFlagArray) {
                                record.set(genderFlagArray[access], false);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        var url = '../controller/genderController.php?';
                        var sub_url = '';
                        var modified = genderStore.getModifiedRecords();
                        for (var i = 0; i < modified.length; i++) {
                            var dataChanges = modified[i].getChanges();
                            sub_url = sub_url + '&genderId[]=' + modified[i].get('genderId');
                            if (isAdmin == 1) {
    ...
    var gridPanel = new Ext.Panel({
     ...
     // form entry
    var genderCode = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: genderCodeLabel + '<span style=\'color: red;\'>*</span>',
    ...
    var genderDesc = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: genderDescLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'genderDesc',
        name: 'genderDesc',
        id: 'genderDesc',
        allowBlank: false,
        blankText: blankTextLabel,
    ...
    var genderCodeTemp = new Ext.form.Hidden({
        name: 'genderCodeTemp',
        id: 'genderCodeTemp'
    });
    var checkDuplicateCode = new Ext.Button ({
    	...
    	listeners: {
            'click': function(button,e) {
                if (Ext.getCmp('genderCode').getValue().length > 0) {
                    Ext.Ajax.request({
                        ...
                        params: {
                            method: 'duplicate',
                            leafId: leafId,
                            genderCode	: Ext.getCmp('genderCode').getValue()
                        },
                        success: function(response, options) {
                            jsonResponse = Ext.decode(response.responseText);
                            if (jsonResponse.success == true) {
                                if (jsonResponse.total > 0) {
                                    if (Ext.getCmp('genderCodeTemp').getValue() != Ext.getCmp('genderCode').getValue()) {
                                        duplicate = 1;
                                        duplicateMessageLabel = duplicateMessageLabel + Ext.util.Format.uppercase(Ext.getCmp('genderCode').getValue()) + ':' + +Ext.util.Format.uppercase(jsonResponse.religionDesc);
                                        Ext.MessageBox.alert(systemErrorLabel, duplicateMessageLabel);
                                        Ext.getCmp('genderCode').setValue('');
                                    } else {
                                    	Ext.MessageBox.alert(systemErrorLabel, jsonResponse.message);
                                    }
                                }
    ...
    var genderId = new Ext.form.Hidden({
        name: 'genderId',
        id: 'genderId'
    }); // end form entry
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            ...
            items: [genderId,{
				...
				items:[genderCode,checkDuplicateCode]
}, genderDesc,genderCodeTemp]
        },
        {
            xtype: 'fieldset',
        ...
        },
        {
            ...
            iconCls: 'new',
            handler: function() {
                var id = Ext.getCmp('genderId').getValue();
                var method = 'create';
                formPanel.getForm().submit({
                    ...
                    success: function(form, action) {
                        if (action.result.success == true) {
                            ...
                            Ext.getCmp('genderId').setValue(action.result.genderId);
                        } else {
                            Ext.MessageBox.alert(systemErrorLabel, action.result.message);
                        }
                    },
                    ...
                    fn: function(response) {
                        if ('yes' == response) {
                            Ext.Ajax.request({
                                ...
                                params: {
                                    method: 'delete',
                                    genderId: Ext.getCmp('genderId').getValue(),
                                    leafId: leafId,
                                    isAdmin: isAdmin
                                },
                                success: function(response, options) {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        genderId: Ext.getCmp('firstRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('firstRecord').getValue() >= 1) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            genderId: Ext.getCmp('previousRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            genderId: Ext.getCmp('nextRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
        ...
        },
        {
            ...
            handler: function() {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        genderId: Ext.getCmp('lastRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
                ...
                if (Ext.getCmp('genderId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        url: '../controller/genderController.php',
                        method: 'POST',
                        waitTitle: systemLabel,
