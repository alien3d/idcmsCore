Ext.onReady(function() {
    ...
    var staffByProxy = new Ext.data.HttpProxy({
        url: '../controller/educationController.php?',
        method: 'GET',
    ...
    var educationProxy = new Ext.data.HttpProxy({
        url: '../controller/educationController.php',
        method: 'POST',
        success: function(response, options) {
            jsonResponse = Ext.decode(response.responseText);
    ...
    var educationFilters = new Ext.ux.grid.GridFilters({
        encode: false,
        local: false,
        filters: [ {
            type: 'list',
    ...
    var educationColumnModel = [new Ext.grid.RowNumberer(), {
        dataIndex: 'educationCode',
        header: educationCodeLabel,
        sortable: true,
        hidden: false,
        width : 200
    },{
        dataIndex: 'educationDesc',
        header: educationDescLabel,
        sortable: true,
        hidden: false,
        width : 200
    },
    ...
    var educationFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
    var educationGrid = new Ext.grid.GridPanel({
        ...
        autoHeight: false,
        height: 400,
        columns: educationColumnModel,
        plugins: [educationFilters],
        selModel: new Ext.grid.RowSelectionModel({
            singleSelect: true
        }),
        viewConfig: {
        ...
        listeners: {
            'rowclick': function(object, rowIndex, e) {
                var record = educationStore.getAt(rowIndex);
                ...
                formPanel.form.load({
                    ...
                    params: {
                        method: 'read',
                        mode: 'update',
                        educationId: record.data.educationId,
                        leafId: leafId,
                        isAdmin: isAdmin
                    },
                    success: function(form, action) {
                        Ext.getCmp('educationDescTemp').setValue(record.data.educationDesc);
        ...
        tbar: {
            items: [{
                ...
                listeners: {
                    'click': function(button,e) {
                        educationStore.each(function(record,fn,scope) {
                            for (var access in educationFlagArray) {
                                record.set(educationFlagArray[access], true);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        educationStore.each(function(record,fn,scope) {
                            for (var access in educationFlagArray) {
                                record.set(educationFlagArray[access], false);
                            }
                        });
                    }
                }
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        var url = '../controller/educationController.php?';
                        var sub_url = '';
                        var modified = educationStore.getModifiedRecords();
                        for (var i = 0; i < modified.length; i++) {
                            var dataChanges = modified[i].getChanges();
                            sub_url = sub_url + '&educationId[]=' + modified[i].get('educationId');
                            if (isAdmin == 1) {
    ...
    var gridPanel = new Ext.Panel({
     ...
     // form entry
    var educationCode = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: familyCodeLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'educationCode',
        name: 'educationCode',
        id: 'educationCode',
        allowBlank: false,
    ...
    var educationDesc = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: educationDescLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'educationDesc',
        name: 'educationDesc',
        id: 'educationDesc',
        allowBlank: false,
        blankText: blankTextLabel,
    ...
    var educationCodeTemp = new Ext.form.Hidden({
        name: 'educationCodeTemp',
        id: 'educationCodeTemp'
    });
    var checkDuplicateCode = new Ext.Button ({
    	...
    	listeners: {
            'click': function(button,e) {
                if (Ext.getCmp('educationCode').getValue().length > 0) {
                    Ext.Ajax.request({
                        ...
                        params: {
                            method: 'duplicate',
                            leafId: leafId,
                            educationCode	: Ext.getCmp('educationCode').getValue()
                        },
                        success: function(response, options) {
                            jsonResponse = Ext.decode(response.responseText);
                            if (jsonResponse.success == true) {
                                if (jsonResponse.total > 0) {
                                    if (Ext.getCmp('educationCodeTemp').getValue() != Ext.getCmp('educationCode').getValue()) {
                                        duplicate = 1;
                                        duplicateMessageLabel = duplicateMessageLabel + Ext.util.Format.uppercase(Ext.getCmp('educationCode').getValue()) + ':' + +Ext.util.Format.uppercase(jsonResponse.religionDesc);
                                        Ext.MessageBox.alert(systemErrorLabel, duplicateMessageLabel);
                                        Ext.getCmp('educationCode').setValue('');
                                    } else {
                                    	Ext.MessageBox.alert(systemErrorLabel, jsonResponse.message);
                                    }
                                }
    ...
    var educationId = new Ext.form.Hidden({
        name: 'educationId',
        id: 'educationId'
    }); // end form entry
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            ...
            items: [educationId,{
				...
				items:[educationCode,checkDuplicateCode]
}, educationDesc,educationCodeTemp]
        },
        {
            xtype: 'fieldset',
        ...
        },
        {
            ...
            iconCls: 'new',
            handler: function() {
                var id = Ext.getCmp('educationId').getValue();
                var method = 'create';
                formPanel.getForm().submit({
                    ...
                    success: function(form, action) {
                        if (action.result.success == true) {
                            ...
                            Ext.getCmp('educationId').setValue(action.result.educationId);
                        } else {
                            Ext.MessageBox.alert(systemErrorLabel, action.result.message);
                        }
                    },
                    ...
                    fn: function(response) {
                        if ('yes' == response) {
                            Ext.Ajax.request({
                                ...
                                params: {
                                    method: 'delete',
                                    educationId: Ext.getCmp('educationId').getValue(),
                                    leafId: leafId,
                                    isAdmin: isAdmin
                                },
                                success: function(response, options) {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        educationId: Ext.getCmp('firstRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('firstRecord').getValue() >= 1) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            educationId: Ext.getCmp('previousRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            educationId: Ext.getCmp('nextRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
        ...
        },
        {
            ...
            handler: function() {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        educationId: Ext.getCmp('lastRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
                ...
                if (Ext.getCmp('educationId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        url: '../controller/educationController.php',
                        method: 'POST',
                        waitTitle: systemLabel,
