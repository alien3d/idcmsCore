					...
					Ext.calendar,
					{
						Date : {
							diffDays : function(b, a) {
								day = 1000 * 60 * 60 * 24;
								diff = a.clearTime(true).getTime()
										- b.clearTime(true).getTime();
...
Ext.calendar.DayHeaderTemplate = function(a) {
	...
	Ext.calendar.DayHeaderTemplate.superclass.constructor
			.call(
					this,
					...
					'<td class="ext-cal-gutter-rt"></td>', "</tr>", "</tobdy>",
					"</table>", "</div>")
};
Ext.extend(Ext.calendar.DayHeaderTemplate, Ext.XTemplate, {
	applyTemplate : function(a) {
		return Ext.calendar.DayHeaderTemplate.superclass.applyTemplate.call(
				this, {
...
Ext.calendar.DayBodyTemplate = function(a) {
	Ext.apply(this, a);
	Ext.calendar.DayBodyTemplate.superclass.constructor
			.call(
					this,
...
Ext.calendar.DayViewTemplate = function(a) {
	...
	Ext.calendar.DayViewTemplate.superclass.constructor.call(this,
			'<div class="ext-cal-inner-ct">', "{headerTpl}", "{bodyTpl}",
			"</div>")
};
Ext.extend(Ext.calendar.DayViewTemplate, Ext.XTemplate, {
	applyTemplate : function(a) {
		return Ext.calendar.DayViewTemplate.superclass.applyTemplate.call(this,
				{
					headerTpl : this.headerTpl.apply(a),
					bodyTpl : this.bodyTpl.apply(a)
				})
	}
});
...
Ext.calendar.BoxLayoutTemplate = function(a) {
	...
	var b = this.showWeekLinks ? '<div id="{weekLinkId}" class="ext-cal-week-link">{weekNum}</div>'
			: "";
	Ext.calendar.BoxLayoutTemplate.superclass.constructor
			.call(
					this,
					'<tpl for="weeks">',
...
Ext
		.extend(
				Ext.calendar.BoxLayoutTemplate,
				Ext.XTemplate,
				{
					applyTemplate : function(e) {
						Ext.apply(this, e);
						var n = 0, m = "", h = true, j = false, f = false, g = false, i = false, a = [ [] ], l = new Date()
								.clearTime(), c = this.viewStart.clone(), b = this.startDate
								.getMonth();
						for (; n < this.weekCount || this.weekCount == -1; n++) {
							...
							for ( var k = 0; k < this.dayCount; k++) {
								...
								if (c.getDay() == 1) {
									a[n].weekNum = this.showWeekNumbers ? c
											.format("W") : "&nbsp;";
									a[n].weekLinkId = "ext-cal-week-"
											+ c.format("Ymd")
								}
								if (f) {
									if (j) {
										...
										m = c
												.format(this.dayCount == 1 ? "l, F j, Y"
														: (h ? "M j, Y" : "M j"))
									}
								} else {
									var p = (n == 0 && this.showHeader !== true) ? "D j"
											: "j";
								...
								a[n]
										.push({
											...
											titleCls : "ext-cal-dtitle "
													+ (j ? " ext-cal-dtitle-today"
															: "")
													+ (n == 0 ? " ext-cal-dtitle-first"
															: "")
													+ (g ? " ext-cal-dtitle-prev"
															: "")
													+ (i ? " ext-cal-dtitle-next"
															: ""),
											cellCls : "ext-cal-day "
													+ (j ? " ext-cal-day-today"
															: "")
													+ (k == 0 ? " ext-cal-day-first"
															: "")
													+ (g ? " ext-cal-day-prev"
															: "")
													+ (i ? " ext-cal-day-next"
						...
						return Ext.calendar.BoxLayoutTemplate.superclass.applyTemplate
								.call(this, {
									weeks : a
								})
					},
					getTodayText : function() {
						var b = new Date().format("l, F j, Y"), c = this.showTodayText !== false ? this.todayText
								: "", a = this.showTime !== false ? ' <span id="'
								+ this.id
								+ '-clock" class="ext-cal-dtitle-time">'
								+ new Date().format("g:i a") + "</span>"
								: "", e = c.length > 0 || a.length > 0 ? " &mdash; "
								: "";
						if (this.dayCount == 1) {
							return b + e + c + a
						}
...
Ext.calendar.MonthViewTemplate = function(a) {
	...
	var b = this.showWeekLinks ? '<div class="ext-cal-week-link-hd">&nbsp;</div>'
			: "";
	Ext.calendar.MonthViewTemplate.superclass.constructor
			.call(
					this,
...
Ext.extend(Ext.calendar.MonthViewTemplate, Ext.XTemplate, {
	applyTemplate : function(f) {
		...
		return Ext.calendar.MonthViewTemplate.superclass.applyTemplate.call(
				this, {
					days : g,
					weeks : e,
					extraClasses : a
...
Ext.dd.ScrollManager = function() {
	var c = Ext.dd.DragDropMgr, f = {}, b = null, i = {}, h = function(l) {
		...
		if (c.dragCurrent) {
			var l = Ext.dd.ScrollManager, m = i.el.ddScrollConfig ? i.el.ddScrollConfig.increment
					: l.increment;
			if (!l.animate) {
		...
		i.el = m;
		i.dir = l;
		var o = (m.ddScrollConfig && m.ddScrollConfig.frequency) ? m.ddScrollConfig.frequency
				: Ext.dd.ScrollManager.frequency, n = m.ddScrollConfig ? m.ddScrollConfig.ddGroup
				: undefined;
...
Ext.calendar.DragZone = Ext.extend(Ext.dd.DragZone, {
	...
	getDragData : function(b) {
		...
		if (a) {
			...
			return {
				type : "eventdrag",
				ddel : a,
				eventStart : c.data[Ext.calendar.EventMappings.StartDate.name],
				eventEnd : c.data[Ext.calendar.EventMappings.EndDate.name],
				proxy : this.proxy
			}
		}
	...
	onInitDrag : function(a, e) {
		if (this.dragData.ddel) {
			var b = this.dragData.ddel.cloneNode(true), c = Ext.fly(b).child(
					"dl");
		...
		.extend(
				Ext.dd.DropZone,
				{
					...
					onNodeOver : function(f, k, j, h) {
						var a = Ext.calendar.Date, b = h.type == "eventdrag" ? f.date
					...
					shim : function(a, f) {
						this.currWeek = -1;
						var b = a.clone(), g = 0, e, h, c = Ext.calendar.Date
								.diffDays(b, f) + 1;
					...
					onNodeDrop : function(h, a, f, c) {
						if (h && c) {
							if (c.type == "eventdrag") {
								var g = this.view.getEventRecordFromEl(c.ddel), b = Ext.calendar.Date
										.copyTime(
												g.data[Ext.calendar.EventMappings.StartDate.name],
												h.date);
	...
	getDragData : function(c) {
		...
		if (a) {
			...
			return {
				type : "eventresize",
				ddel : b.dom,
				eventStart : f.data[Ext.calendar.EventMappings.StartDate.name],
				eventEnd : f.data[Ext.calendar.EventMappings.EndDate.name],
				proxy : this.proxy
			}
		}
...
Ext.calendar.DayViewDropZone = Ext.extend(Ext.calendar.DropZone,
		{
			...
			onNodeOver : function(c, k, j, g) {
				...
				if (g.type == "caldrag") {
					...
					h.height = Math.ceil(Math.abs(j.xy[1] - h.y)
							/ c.timeBox.height)
							* c.timeBox.height;
					if (j.xy[1] < h.y) {
					...
					this.dragStartDate = Ext.calendar.Date.min(
							this.dragCreateDt, p);
					this.dragEndDate = i
							|| Ext.calendar.Date.max(this.dragCreateDt, p);
					b = this.dragStartDate.format("g:ia-")
							+ this.dragEndDate.format("g:ia");
				} else {
					o = Ext.get(g.ddel);
					m = o.parent().parent();
					h = o.getBox();
			...
			onNodeDrop : function(g, a, c, b) {
				...
				if (g && b) {
					...
					if (b.type == "caldrag") {
						...
						this.view.onCalendarEndDrag(this.dragStartDate,
								this.dragEndDate, this.onCalendarDragComplete
										.createDelegate(this));
						return true
					}
				}
...
(function() {
	...
	Ext.calendar.EventRecord = Ext.data.Record.create([ a.EventId,
			a.CalendarId, a.Title, a.StartDate, a.EndDate, a.Location, a.Notes,
			a.Url, a.IsAllDay, a.Reminder, a.IsNew ]);
	Ext.calendar.EventRecord.reconfigure = function() {
		Ext.calendar.EventRecord = Ext.data.Record.create([ a.EventId,
				a.CalendarId, a.Title, a.StartDate, a.EndDate, a.Location,
				a.Notes, a.Url, a.IsAllDay, a.Reminder, a.IsNew ])
	}
})();
Ext.calendar.MonthDayDetailView = Ext
		.extend(
				Ext.BoxComponent,
				{
					initComponent : function() {
						Ext.calendar.CalendarView.superclass.initComponent
								.call(this);
						this.addEvents({
							eventsrendered : true
						});
					...
					afterRender : function() {
						...
						Ext.calendar.MonthDayDetailView.superclass.afterRender
								.call(this);
						this.el.on({
					...
					refresh : function() {
						...
						evts = this.store
								.queryBy(
										function(i) {
											var f = this.date.clearTime(true)
													.getTime(), e = i.data[Ext.calendar.EventMappings.StartDate.name]
													.clearTime(true).getTime(), g = (f == e), h = false;
											if (!g) {
												var c = i.data[Ext.calendar.EventMappings.EndDate.name]
														.clearTime(true)
														.getTime();
												h = e < f && c >= f
											}
											...
											e._renderAsAllDay = e[f.IsAllDay.name]
													|| Ext.calendar.Date
															.diffDays(
																	e[f.StartDate.name],
																	e[f.EndDate.name]) > 0;
											e.spanLeft = Ext.calendar.Date
													.diffDays(
															e[f.StartDate.name],
															this.date) > 0;
											e.spanRight = Ext.calendar.Date
													.diffDays(this.date,
															e[f.EndDate.name]) > 0;
...
Ext.calendar.CalendarPicker = Ext.extend(Ext.form.ComboBox, {
	...
	initComponent : function() {
		...
		this.tpl = this.tpl
				|| '<tpl for="."><div class="x-combo-list-item ext-color-{'
				+ this.valueField
...
Ext.calendar.WeekEventRenderer = function() {
	...
	return {
		render : function(m) {
			var g = 0, b = m.eventGrid, k = m.viewStart.clone(), l = m.tpl, q = m.maxEventsPerDay != undefined ? m.maxEventsPerDay
					: 999, s = m.weekCount < 1 ? 6 : m.weekCount, n = m.weekCount == 1 ? m.dayCount
					: 7, p;
			for (; g < s; g++) {
				if (!b[g] || b[g].length == 0) {
					if (s == 1) {
						...
						p = {
							tag : "td",
							cls : "ext-cal-ev",
							id : m.id + "-empty-0-day-" + k.format("Ymd"),
							html : "&nbsp;"
						};
						if (n > 1) {
							p.colspan = n
					...
					var f, x = 0, c = b[g], z = k.clone(), h = z.add(Date.DAY,
							n).add(Date.MILLI, -1);
					for (; x < n; x++) {
						if (c[x]) {
							...
							for (; v < e; v++) {
								...
								if (!j.isSpan || j.isSpanStart) {
									...
									u._weekIndex = g;
									u._renderAsAllDay = u[Ext.calendar.EventMappings.IsAllDay.name]
											|| j.isSpanStart;
									u.spanLeft = u[Ext.calendar.EventMappings.StartDate.name]
											.getTime() < z.getTime();
									u.spanRight = u[Ext.calendar.EventMappings.EndDate.name]
											.getTime() > h.getTime();
									u.spanCls = (u.spanLeft ? (u.spanRight ? "ext-cal-ev-spanboth"
											: "ext-cal-ev-spanleft")
											: (u.spanRight ? "ext-cal-ev-spanright"
													: ""));
									f = a(m.id, g, v);
									p = {
										tag : "td",
										cls : "ext-cal-ev",
										cn : l.apply(m.templateDataFn(u))
									};
									var i = Ext.calendar.Date
											.diffDays(
													k,
													u[Ext.calendar.EventMappings.EndDate.name]) + 1, t = Math
											.min(i, n - x);
									if (t > 1) {
										p.colspan = t
									}
							...
							if (f) {
								p = {
									tag : "td",
									cls : "ext-cal-ev",
									id : m.id + "-empty-day-" + k.format("Ymd")
								};
...
Ext.calendar.CalendarView = Ext
		.extend(
				Ext.BoxComponent,
				{
					...
					afterRender : function() {
						Ext.calendar.CalendarView.superclass.afterRender
								.call(this);
						this.renderTemplate();
						if (this.store) {
					...
					getWeekCount : function() {
						var a = Ext.calendar.Date.diffDays(this.viewStart,
								this.viewEnd);
							...
							for (d = 0; d < this.dayCount; d++) {
								if (b.getCount() > 0) {
									var a = b
											.filterBy(
													function(k) {
														var j = (f.getTime() == k.data[Ext.calendar.EventMappings.StartDate.name]
																.clearTime(true)
																.getTime());
														var i = (c == 0
																&& d == 0 && (f > k.data[Ext.calendar.EventMappings.StartDate.name]));
														return j || i
													}, this);
					...
					prepareEventGrid : function(c, b, g) {
						...
						c
								.each(
										function(h) {
											var j = Ext.calendar.EventMappings, i = Ext.calendar.Date
													.diffDays(
															Ext.calendar.Date
																	.max(
																			this.viewStart,
																			h.data[j.StartDate.name]),
															Ext.calendar.Date
																	.min(
																			this.viewEnd,
																			h.data[j.EndDate.name])) + 1;
											if (i > 1
													|| Ext.calendar.Date
															.diffDays(
																	h.data[j.StartDate.name],
					...
					prepareEventGridSpans : function(i, a, h, g, j, k) {
						var f = h, b = g, l = this.findEmptyRowIndex(h, g, k), e = this.viewStart
								.clone();
						var c = {
							event : i,
							isSpan : true,
					...
					onEventDrop : function(c, a) {
						if (Ext.calendar.Date
								.compare(
										c.data[Ext.calendar.EventMappings.StartDate.name],
										a) === 0) {
							return
						}
						var b = a.getTime()
						...
						c.set(Ext.calendar.EventMappings.EndDate.name,
								c.data[Ext.calendar.EventMappings.EndDate.name]
										.add(Date.MILLI, b));
					...
					onCalendarEndDrag : function(e, a, b) {
						...
						this.fireEvent("rangeselect", this, c,
								this.onCalendarEndDragComplete.createDelegate(
										this, [ b ]))
					},
						...
						if (a == Ext.data.Record.COMMIT) {
							...
							if (this.enableFx && this.enableUpdateFx) {
								this
										.doUpdateFx(
												this
														.getEventEls(c.data[Ext.calendar.EventMappings.EventId.name]),
												{
													scope : this
												})
							}
					...
					onAdd : function(c, a, b) {
						...
						if (this.enableFx && this.enableAddFx) {
							this
									.doAddFx(
											this
													.getEventEls(e.data[Ext.calendar.EventMappings.EventId.name]),
											{
												scope : this
											})
						}
					...
					onRemove : function(a, b) {
						...
						if (this.enableFx && this.enableRemoveFx) {
							this
									.doRemoveFx(
											this
													.getEventEls(b.data[Ext.calendar.EventMappings.EventId.name]),
											{
												remove : true,
												scope : this,
					...
					getEventEls : function(b) {
						var a = Ext.select(this.getEventSelectorCls(this
								.getEventId(b), true), false, this.el.id);
					...
					isEventVisible : function(i) {
						var b = this.viewStart.getTime(), e = this.viewEnd
								.getTime(), g = Ext.calendar.EventMappings, j = (i.data ? i.data[g.StartDate.name]
								: i[g.StartDate.name]).getTime(), h = (i.data ? i.data[g.EndDate.name]
								: i[g.EndDate.name]).add(Date.SECOND, -1)
								.getTime(), c = (j >= b && j <= e), a = (h >= b && h <= e), f = (j < b && h > e);
						return (c || a || f)
					},
					isOverlapping : function(l, k) {
						var j = l.data ? l.data : l, i = k.data ? k.data : k, g = Ext.calendar.EventMappings, c = j[g.StartDate.name]
								.getTime(), h = j[g.EndDate.name].add(
								Date.SECOND, -1).getTime(), b = i[g.StartDate.name]
								.getTime(), f = i[g.EndDate.name].add(
								Date.SECOND, -1).getTime();
						if (h < c) {
							h = c
						}
						if (f < b) {
					...
					setViewBounds : function(a) {
						...
						case 0:
						case 1:
							this.viewStart = this.dayCount < 7 ? e : e.add(
					...
					movePrev : function(a) {
						var b = Ext.calendar.Date.diffDays(this.viewStart,
								this.viewEnd) + 1;
					...
					setStore : function(a, b) {
						if (!b && this.store) {
							this.store.un("datachanged", this.onDataChanged,
									this);
					...
					getEventRecord : function(b) {
						var a = this.store.find(
								Ext.calendar.EventMappings.EventId.name, b);
					...
					onClick : function(c, a) {
						...
						if (b) {
							...
							this.fireEvent("eventclick", this, this
									.getEventRecord(f), b);
							return true
						}
					},
						...
						if (f) {
							if (f.id
								...
								var i = this.getDateFromId(f.id,
										this.dayElIdDelimiter), a = Ext.get(j
...
Ext.calendar.MonthView = Ext
		.extend(
				Ext.calendar.CalendarView,
				{
					showTime : true,
					showTodayText : true,
					todayText : "Today",
					...
					initComponent : function() {
						Ext.calendar.MonthView.superclass.initComponent
								.call(this);
						this.addEvents({
							dayclick : true,
							weekclick : true,
					...
					afterRender : function() {
						...
						Ext.calendar.MonthView.superclass.afterRender
								.call(this)
					},
					onResize : function() {
					...
					getTemplateEventData : function(b) {
						var e = Ext.calendar.EventMappings, a = this
								.getEventSelectorCls(b[e.EventId.name]), c = b[e.Title.name];
						return Ext
								.applyIf(
										{
											_selectorCls : a,
											_colorCls : "ext-color-"
													+ (b[e.CalendarId.name] ? b[e.CalendarId.name]
															: "default")
													+ (b._renderAsAllDay ? "-ad"
															: ""),
											_elId : a + "-" + b._weekIndex,
											_isRecurring : b.Recurrence
													&& b.Recurrence != "",
											_isReminder : b[e.Reminder.name]
													&& b[e.Reminder.name] != "",
											Title : (b[e.IsAllDay.name] ? ""
													: b[e.StartDate.name]
															.format("g:ia "))
													+ (!c || c.length == 0 ? "(No title)"
															: c)
										}, b)
					},
					refresh : function() {
					...
					getDaySize : function(f) {
						var c = this.el.getBox(), a = c.width / this.dayCount, b = c.height
								/ this.getWeekCount();
						if (f) {
							var e = this.el.select(".ext-cal-dtitle").first()
									.parent("tr");
					...
					getEventHeight : function() {
						if (!this.eventHeight) {
							...
							this.eventHeight = a ? a.parent("tr").getHeight()
									: 18
						}
					...
					getMaxEventsPerDay : function() {
						var b = this.getDaySize(true).height, c = this
								.getEventHeight(), a = Math.max(Math
								.floor((b - c) / c), 0);
						return a
					},
					getDayAt : function(a, i) {
						var f = this.el.getBox(), b = this.getDaySize(), c = Math
								.floor(((a - f.x) / b.width)), g = Math
								.floor(((i - f.y) / b.height)), h = (g * 7) + c, e = this.viewStart
								.add(Date.DAY, h);
						return {
							date : e,
							el : this.getDayEl(e)
					...
					onDetailViewUpdated : function(h, c, i) {
						var b = this.detailPanel, f = b.getFrameHeight(), j = this
								.getEventHeight(), a = f + (i * j) + 3, g = this
					...
					onClick : function(g, a) {
						...
						if (Ext.calendar.MonthView.superclass.onClick.apply(
								this, arguments)) {
							return
						}
						if (this.dropZone) {
						...
						if (b) {
							...
							this.fireEvent("weekclick", this, Date.parseDate(c,
									"Ymd"));
							return
						}
					...
					handleDayMouseEvent : function(f, a, c) {
						...
						Ext.calendar.MonthView.superclass.handleDayMouseEvent
	...
	onClick : function(g, a) {
		...
		if (b) {
			if (b.id && b.id.indexOf(this.dayElIdDelimiter) > -1) {
				f = b.id.split(this.dayElIdDelimiter);
				c = f[f.length - 1];
				this.fireEvent("dayclick", this, Date.parseDate(c, "Ymd"),
						true, Ext.get(this.getDayId(c)));
				return
			}
		}
...
Ext.calendar.DayBodyView = Ext
		.extend(
				Ext.calendar.CalendarView,
				{
					dayColumnElIdDelimiter : "-day-col-",
					initComponent : function() {
						Ext.calendar.DayBodyView.superclass.initComponent
								.call(this);
						this.addEvents({
							eventresize : true,
							dayclick : true
					...
					initDD : function() {
						...
						this.dragZone = new Ext.calendar.DayViewDragZone(
								this.el, Ext.apply({
									view : this,
									containerScroll : true
								}, a));
						this.dropZone = new Ext.calendar.DayViewDropZone(
								this.el, Ext.apply({
									view : this
								}, a))
					},
					...
					afterRender : function() {
						...
						Ext.calendar.DayBodyView.superclass.afterRender
								.call(this);
						this.scrollTo(7 * 42)
					},
					forceSize : Ext.emptyFn,
					...
					getTemplateEventData : function(b) {
						var a = this
								.getEventSelectorCls(b[Ext.calendar.EventMappings.EventId.name]), c = {}, f = Ext.calendar.EventMappings;
						this.getTemplateEventBox(b);
						c._selectorCls = a;
						c._colorCls = "ext-color-" + b[f.CalendarId.name]
								+ (b._renderAsAllDay ? "-ad" : "");
						c._elId = a + (b._weekIndex ? "-" + b._weekIndex : "");
						c._isRecurring = b.Recurrence && b.Recurrence != "";
						c._isReminder = b[f.Reminder.name]
								&& b[f.Reminder.name] != "";
						var e = b[f.Title.name];
						c.Title = (b[f.IsAllDay.name] ? ""
								: b[f.StartDate.name].format("g:ia "))
								+ (!e || e.length == 0 ? "(No title)" : e);
						return Ext.applyIf(c, b)
					},
					getTemplateEventBox : function(c) {
						var g = 0.7, h = c[Ext.calendar.EventMappings.StartDate.name], b = c[Ext.calendar.EventMappings.EndDate.name], f = h
								.getHours()
								* 60 + h.getMinutes(), a = b.getHours() * 60
								+ b.getMinutes(), e = a - f;
						c._left = 0;
						c._width = 100;
					...
					renderItems : function() {
						...
						for (; p < this.dayCount; p++) {
							...
							for (; o < k; o++) {
								...
								s.push({
									data : this.getTemplateEventData(r),
						...
						for (; h < b; h++) {
							...
							for (e = 0; e < b; e++) {
								...
								if (this.isOverlapping(evt, evt2)) {
									evt._overlap = evt._overlap == undefined ? 1
											: evt._overlap + 1;
									if (h < e) {
					...
					getDayAt : function(n, j) {
						var f = ".ext-cal-body-ct", h = this.el.child(
								".ext-cal-day-times").getWidth(), r = this.el
								.getBox(), m = this.getDaySize(false), o = n
								- r.x - h, a = Math.floor(o / m.width), l = this.el
								.getScroll(), q = this.el
								.child(".ext-cal-bg-row"), p = q.getHeight() / 2, k = j
								- r.y - p + l.top, i = Math.max(0, Math.ceil(k
								/ p)), b = i * 30, e = this.viewStart.add(
								Date.DAY, a).add(Date.MINUTE, b), c = this
								.getDayEl(e), g = n;
						if (c) {
							g = c.getLeft()
						}
					...
					onClick : function(g, b) {
						if (this.dragPending
								|| Ext.calendar.DayBodyView.superclass.onClick
										.apply(this, arguments)) {
							return
						}
						...
						if (c) {
							if (c.id
								...
								var f = this.getDateFromId(c.id,
										this.dayElIdDelimiter);
								this.fireEvent("dayclick", this, Date
										.parseDate(f, "Ymd"), true, Ext
										.get(this.getDayId(f, true)));
								return
							}
						}
						var a = this.getDayAt(g.xy[0], g.xy[1]);
						if (a && a.date) {
							this.fireEvent("dayclick", this, a.date, false,
									null)
						}
					}
				});
	...
	forceSize : function() {
		(function() {
			var a = this.el.up(".x-panel-body"), c = this.el
					.child(".ext-cal-day-header"), b = a.getHeight()
					- c.getHeight();
...
Ext.calendar.DateRangeField = Ext
		.extend(
				Ext.form.Field,
				{
					toText : "to",
					...
					onRender : function(b, a) {
						if (!this.el) {
							...
							this.fieldCt = new Ext.Container({
								...
								items : [ this.startDate, this.startTime,
										this.toLabel, this.endTime,
										this.endDate, this.allDay ]
							});
						...
						Ext.calendar.DateRangeField.superclass.onRender.call(
								this, b, a)
					},
						...
						if (Ext.isArray(a)) {
							...
							if (Ext.isDate(a)) {
								...
								if (a[Ext.calendar.EventMappings.StartDate.name]) {
									this
											.setDT(
													a[Ext.calendar.EventMappings.StartDate.name],
													"start");
									if (!this
											.setDT(
													a[Ext.calendar.EventMappings.EndDate.name],
													"end")) {
										this
												.setDT(
														a[Ext.calendar.EventMappings.StartDate.name],
									...
									this.allDay
											.setValue(!!a[Ext.calendar.EventMappings.IsAllDay.name])
								}
							}
						}
					},
					...
					beforeDestroy : function() {
						...
						Ext.calendar.DateRangeField.superclass.beforeDestroy
								.call(this)
					},
...
Ext.calendar.EventEditForm = Ext
		.extend(
				Ext.form.FormPanel,
				{
					labelWidth : 65,
					...
					initComponent : function() {
						...
						Ext.calendar.EventEditForm.superclass.initComponent
								.call(this)
					},
					loadRecord : function(a) {
						...
						if (this.calendarStore) {
							this.form
									.setValues({
										calendar : a.data[Ext.calendar.EventMappings.CalendarId.name]
									})
						}
					...
					updateRecord : function() {
						...
						this.activeRecord
								.set(Ext.calendar.EventMappings.StartDate.name,
										a[0]);
						this.activeRecord.set(
								Ext.calendar.EventMappings.EndDate.name, a[1]);
						this.activeRecord.set(
								Ext.calendar.EventMappings.IsAllDay.name, a[2])
					},
					onCancel : function() {
						this.cleanup(true);
					...
					onSave : function() {
						...
						this.fireEvent(this.isAdd ? "eventadd" : "eventupdate",
...
Ext
		.extend(
				Ext.calendar.EventEditWindow,
				Ext.Window,
				{
					newId : 10000,
					initComponent : function() {
						Ext.calendar.EventEditWindow.superclass.initComponent
								.call(this);
					...
					afterRender : function() {
						Ext.calendar.EventEditWindow.superclass.afterRender
								.call(this);
						...
						Ext.get("tblink").on(
								...
								function(a) {
									a.stopEvent();
									this.updateRecord();
									this.fireEvent("editdetails", this,
											this.activeRecord)
								}, this)
					},
					show : function(c, e) {
						var i = (Ext.isIE8 && Ext.isStrict) ? null : e;
						Ext.calendar.EventEditWindow.superclass.show.call(this,
								i, function() {
									Ext.getCmp("title").focus(false, 100)
								});
						Ext.getCmp("delete-btn")[c.data
								&& c.data[Ext.calendar.EventMappings.EventId.name] ? "show"
								: "hide"]();
						...
						if (c.data) {
							...
							this.isAdd = true;
							this.setTitle(this.titleTextAdd);
							var k = Ext.calendar.EventMappings, a = k.EventId.name, b = c[k.StartDate.name], g = c[k.EndDate.name]
									|| b.add("h", 1);
							h = new Ext.calendar.EventRecord();
							h.data[k.EventId.name] = this.newId++;
							h.data[k.StartDate.name] = b;
							h.data[k.EndDate.name] = g;
							h.data[k.IsAllDay.name] = !!c[k.IsAllDay.name]
									|| b.getDate() != g.clone().add(Date.MILLI,
											1).getDate();
						...
						if (this.calendarStore) {
							Ext
									.getCmp("calendar")
									.setValue(
											h.data[Ext.calendar.EventMappings.CalendarId.name])
						}
					...
					updateRecord : function() {
						var a = this.formPanel.form, b = Ext.getCmp(
								"date-range").getValue(), c = Ext.calendar.EventMappings;
						...
						this.activeRecord.set(c.CalendarId.name,
								this.formPanel.form.findField("calendar")
										.getValue())
					},
					onSave : function() {
...
Ext.calendar.CalendarPanel = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {
		...
		this.activeItem = this.activeItem === undefined ? a
				: (this.activeItem > a ? a : this.activeItem);
	...
	initEventRelay : function(a) {
		...
		a.listeners.afterrender = {
			fn : function(b) {
				this.relayEvents(b, [ "eventsrendered", "eventclick",
						"eventover", "eventout", "dayclick", "eventmove",
						"datechange", "rangeselect", "eventdelete",
						"eventresize", "initdrag" ])
			},
			scope : this,
			single : true
		}
