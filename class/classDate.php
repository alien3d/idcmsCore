<?php	class dateClass {
	...
	function forwardDate($date_receive,$type) {
		...
		$this->totalDayInMonth=date('t',mktime('0','0','0',$this->month,$this->day,$this->year));
		if($this->type=='day') {
			...
			$returnDate=trim($this->year."-".$this->month."-".$this->day);
	...
	function firstdayweek($date) {
		...
		for ($i=$lowEnd; $i<=$highEnd; $i++) {
			$WeekDate[$weekday]=date("Y-m-d",mktime(0, 0, 0, date("m",$date) , date("d",$date)+$i +1, date("Y",$date)));
			$weekday++;
		}
		echo $WeekDate[0] ;
	}
