...
class Vendor
{
	...
	public function connect($connection, $username, $database, $password)
	{
		...
		$this->link         = cubrid_connect($this->connection, $this->username, $this->password, $this->databaseName, $this->port, $this->socket);
		if (!$this->link) {
			$this->execute = 'fail';
			if (cubrid_connect_errno()) {
				$this->responce = cubrid_connect_errno();
			...
			$resources = cubrid_select_db($this->link, $this->databaseName);
			if (!$resources) {
	...
	{
		cubrid_autocommit($this->link, FALSE);
	}
	/**
	 * query database
	 * @param string $sql
	...
	private function query($sql)
	{
		...
		$this->sql         = $sql;
		$error             = 0;
		$this->result      = cubrid_execute($this->link, $this->sql);
		if (!$this->result) {
			$this->execute 	=	'fail';
			$this->responce = 	"Sql Stament Error" . $this->sql . " \n\r" . cubrid_error($this->link) . " <br> Error Code :x " . cubrid_errno($this->link);
			$error          = 	1;
		}
		if ($error == 1) {
			...
			$result_row = cubrid_execute($this->link, $sql_log);
			if (!$result_row) {
				$this->responce=$sql_log."<br>".cubrid_error($this->link)."<br> Error Code :y ".cubrid_errno($this->link);
			}
		}
	...
	private function module($operation)
	{
		...
		if (!$result) {
			$this->execute     = 'false';
			$this->responce = $sql . cubrid_error($this->link);
			$result_row        = 0;
		} else {
			$result_row = cubrid_num_rows($result);
		}
		if ($result_row == 1) {
			$access = 'Granted';
		} elseif ($result_row == 0) {
		...
		if ($result_row == 0  || $this->log == 1) {
			...
			$test1   = cubrid_execute($this->link, $sql_log);
			if (!$test1) {
				$this->execute     = 'fail';
				$this->responce = $sql_log . "[" . cubrid_error($this->link) . "]";
			}
		}
		return ($result_row);
	}
		...
		if (strlen($sql) > 0) {
			if ($this->module('leafAccessCreateValue') == 1) {
				...
				if($this->audit==1){
					...
					$resultColumn    = cubrid_execute($this->link, $sqlColumn);
					if (!$resultColumn) {
						$this->execute     = 'fail';
					...
					$resultPrevious = cubrid_execute($this->link, $sqlPrevious);
					if (!$resultPrevious) {
						$this->execute     = 'fail';
						$this->responce = cubrid_error($this->link) . "Error Code" . cubrid_errno($this->link);
					} else {
					...
					$resultLogAdvance = cubrid_execute($this->link, $sqlLogAdvance);
					if (!$resultLogAdvance) {
						$this->execute     = 'fail';
			...
			if ($this->module('leafAccessUpdateValue') == 1) {
				if ($this->audit == 1) {
					...
					if ($resultLogAdvance) {
						...
						$logAdvanceId = cubrid_insert_id($this->link); //
					} else {
						$this->execute  = 	'fail';
						$this->responce	=	"error inserting query update insert";
					}
	...
	public function file($sql)
	{
		 ...
		 */
		if (strlen($sql) > 0) {
			$result = cubrid_execute($this->link, $this->sql);
			return $result;
		} else {
			$this->execute     = 'fail';
			$this->responce = "Where's the query forgot Ya!";
	...
	public function numberRows($result = null, $sql = null)
	{
		if ($result) {
			if(cubrid_num_rows($result)) {
				$this->countRecord = cubrid_num_rows($result);
			} else {
				$this->countRecord =0;
			}
		} else {
			if(cubrid_num_rows($this->result)){
				$this->countRecord = cubrid_num_rows($this->result);
			} else {
				$this->countRecord=0;
			}
		}
	...
	public function lastInsertId()
	{
		...
		$this->insertId = cubrid_insert_id($this->link);
	...
	{
		return cubrid_affected_rows($this->link);
	...
	{
		if ($this->result) {
			return @cubrid_fetch_array($this->result);
		}
		if ($result) {
			return @cubrid_fetch_array($result);
		}
	}
	/**
	 *
	...
	public function fetchAssoc($result = null)
	{
		...
		if ($this->result && is_null($result)) {
			return cubrid_fetch_assoc($this->result);
		}
		if ($result) {
	...
	{
		return cubrid_real_escape_string($this->link, $data);
	}
	/**
