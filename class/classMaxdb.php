...
class Vendor
{
	...
	public function connect($connection, $username, $database, $password)
	{
		...
		$this->link         = maxdb_connect($this->connection, $this->username, $this->password, $this->databaseName, $this->port, $this->socket);
		if (!$this->link) {
			$this->execute = 'fail';
			if (maxdb_connect_errno()) {
				$this->responce = maxdb_connect_errno();
			...
			$resources = maxdb_select_db($this->link, $this->databaseName);
			if (!$resources) {
				$this->responce = maxdb_error($this->link) . "Error Code" . maxdb_errno($this->link);
				echo json_encode(array(
                    "success" => false,
	...
	{
		maxdb_autocommit($this->link, FALSE);
	}
	/**
	 * query database
	 * @param string $sql
	...
	private function query($sql)
	{
		...
		$this->sql         = $sql;
		$error             = 0;
		$this->result      = maxdb_query($this->link, $this->sql);
		if (!$this->result) {
			$this->execute 	=	'fail';
			$this->responce = 	"Sql Stament Error" . $this->sql . " \n\r" . maxdb_error($this->link) . " <br> Error Code :x " . maxdb_errno($this->link);
			$error          = 	1;
		}
		if ($error == 1) {
			...
			$result_row = maxdb_query($this->link, $sql_log);
			if (!$result_row) {
				$this->responce=$sql_log."<br>".maxdb_error($this->link)."<br> Error Code :y ".maxdb_errno($this->link);
			}
		}
	...
	private function module($operation)
	{
		...
		$result          = maxdb_query($this->link, $sql);
		if (!$result) {
			$this->execute     = 'false';
			$this->responce = $sql . maxdb_error($this->link);
			$result_row        = 0;
		} else {
			$result_row = maxdb_num_rows($result);
		}
		if ($result_row == 1) {
			$access = 'Granted';
		} elseif ($result_row == 0) {
		...
		if ($result_row == 0  || $this->log == 1) {
			...
			$test1   = maxdb_query($this->link, $sql_log);
			if (!$test1) {
				$this->execute     = 'fail';
				$this->responce = $sql_log . "[" . maxdb_error($this->link) . "]";
			}
		}
		return ($result_row);
	}
		...
		if (strlen($sql) > 0) {
			if ($this->module('leafAccessCreateValue') == 1) {
				...
				if($this->audit==1){
					...
					$resultColumn    = maxdb_query($this->link, $sqlColumn);
					if (!$resultColumn) {
						$this->execute     = 'fail';
					...
					if (!$resultColumn) {
						...
						while (($rowColumn = maxdb_fetch_array($resultColumn)) == TRUE) {
					...
					$resultPrevious = maxdb_query($this->link, $sqlPrevious);
					if (!$resultPrevious) {
						$this->execute     = 'fail';
						$this->responce = maxdb_error($this->link) . "Error Code" . maxdb_errno($this->link);
					} else {

						while (($rowPrevious = maxdb_fetch_array($resultPrevious)) == TRUE) {
					...
					$resultLogAdvance = maxdb_query($this->link, $sqlLogAdvance);
					if (!$resultLogAdvance) {
						$this->execute     = 'fail';
			...
			if ($this->module('leafAccessUpdateValue') == 1) {
				if ($this->audit == 1) {
					...
					if (!$resultColumn) {
						...
						//	echo "Jumlah Rekod".maxdb_num_rows($resultColumn);
						while (($rowColumn = maxdb_fetch_array($resultColumn)) == TRUE) {
					...
					if (!$resultPrevious) {
						...
						// successfully
						//	echo "Jumlah Rekod ".maxdb_num_rows($resultPrevious);
						while (($rowPrevious = maxdb_fetch_array($resultPrevious)) == TRUE) {
							foreach ($fieldValue as $field) {
								$text .= "\"" . $field . "\":\"" . $rowPrevious[$field] . "\",";
								$previous[$field] = $rowPrevious[$field];
					...
					if ($resultLogAdvance) {
						...
						$logAdvanceId = maxdb_insert_id($this->link); //
					} else {
						$this->execute  = 	'fail';
						$this->responce	=	"error inserting query update insert";
					}
				...
				if ($this->audit == 1) {
					...
					FROM 	`" . $this->tableName . "`
					WHERE 	`" . $this->primaryKeyName . "`=\"". $this->primaryKeyValue . "\"";
					$resultCurrent = maxdb_query($this->link, $sqlCurrent);
					if ($resultCurrent) {
						while (($rowCurrent = maxdb_fetch_array($resultCurrent)) == TRUE) {
	...
	public function file($sql)
	{
		 ...
		 */
		if (strlen($sql) > 0) {
			$result = maxdb_query($this->link, $this->sql);
			return $result;
		} else {
			$this->execute     = 'fail';
			$this->responce = "Where's the query forgot Ya!";
	...
	{
		if ($result) {
			if(maxdb_num_rows($result)) {
				$this->countRecord = maxdb_num_rows($result);
			} else {
				$this->countRecord =0;
			}
		} else {
			if(maxdb_num_rows($this->result)){
				$this->countRecord = maxdb_num_rows($this->result);
			} else {
				$this->countRecord=0;
			}
		}
	...
	public function lastInsertId()
	{
		...
		$this->insertId = maxdb_insert_id($this->link);
	...
	{
		return maxdb_affected_rows($this->link);
	...
	{
		if ($this->result) {
			return @maxdb_fetch_array($this->result);
		}
		if ($result) {
			return @maxdb_fetch_array($result);
		}
	}
	/**
	 *
	...
	public function activeRecord($result = null)
	{
		$d = array();
		if ($result) {
			while (($row = maxdb_fetch_assoc($result)) == TRUE) {
				$d[] = $row;
			}
		} else {
			while (($row = maxdb_fetch_assoc($this->result)) == TRUE) {
				$d[] = $row;
			}
		}
		return $d;
	...
	public function fetchAssoc($result = null)
	{
		...
		if ($this->result && is_null($result)) {
			return maxdb_fetch_assoc($this->result);
		}
		if ($result) {
	...
	{
		return maxdb_real_escape_string($this->link, $data);
	}
	/**
	...
	public function quickSearch($tableArray, $filterArray)
	{
		...
		foreach ($tableArray as $tableSearch) {
			...
			$result = maxdb_query($this->link,$sql);
			if (maxdb_num_rows($result) > 0) {
				while (($row = maxdb_fetch_array($result)) == TRUE) {
