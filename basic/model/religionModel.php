...
class ReligionModel extends ValidationClass
{
    ...
    public function execute ()
    {
        ...
        if (isset($_POST['religionDesc'])) {
            $this->setReligionDesc(
            $this->strict($_POST['religionDesc'], 'memo'));
        }
        ...
        for ($i = 0; $i < $this->getTotal(); $i ++) {
            if (isset($_GET['religionId'])) {
                $this->setReligionId(
                $this->strict($_GET['religionId'][$i], 'numeric'), $i, 'array');
            }
            if (isset($_GET['isDefault'])) {
    ...
    public function setReligionId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:setReligionId ?"));
                exit();
            }
    }
    /**
    ...
    public function getReligionId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:getReligionId ?"));
                exit();
            }
    }
    /**
