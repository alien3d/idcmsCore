										...
										listeners : {
											'click' : function(c) {
												...
												for ( var i = 0; i < modified.length; i++) {
													...
													if (isAdmin == 1) {
														if (dataChanges.isActive) {
															sub_url = sub_url
																	+ '&isActive[]='
																	+ record
																			.get('isActive');
														}
														if (dataChanges.isApproved) {
															sub_url = sub_url
																	+ '&isApproved[]='
																	+ record
																			.get('isApproved');
														}
														if (dataChanges.isReview) {
															sub_url = sub_url
																	+ '&isReview[]='
																	+ record
																			.get('isReview');
														}
														if (dataChanges.isPost) {
															sub_url = sub_url
																	+ '&isPost[]='
																	+ record
																			.get('isPost');
