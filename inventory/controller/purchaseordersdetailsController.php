 ...
 * @subpackage purchaseOrdersDetailsv1,v2,v3,v4,v5
...
class PurchaseOrdersDetailsClass extends ConfigClass {
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	AND `purchaseOrdersDetails`.`isActive`		=	1	";
			} else if ($this->q->vendor == self::MSSQL) {
				$this->auditFilter = "	AND [purchaseOrdersDetails].[isActive]		=	1	";
			} else if ($this->q->vendor == self::ORACLE) {
				$this->auditFilter = "	AND PURCHASEORDERDETAILS.ISACTIVE	=	1	";
	...
	function update() {
		...
		if ($total == 0) {
			...
			$this->q->tableName = $this->model->getTableName();
			$this->q->primaryKeyName = $this->model->getPrimaryKeyName();
			$this->q->primaryKeyValue = $this->model->getPurchaseOrderDetailsId(0, 'single');
			$this->q->audit = $this->audit;
			$this->q->update($sql);
			if ($this->q->execute == 'fail') {
				echo json_encode(array("success" => false, "message" => $this->q->responce));
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getPurchaseOrderDetailsId($i, 'array') . "'
	...
	function duplicate() {
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => "Duplicate Record", "purchaseOrdersDetailsDesc" => $row ['purchaseOrdersDetailsDesc']));
			exit();
		} else {
