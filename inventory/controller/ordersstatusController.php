 ...
 * @subpackage ordersstatusv1,v2,v3,v4,v5
...
class OrdersStatusClass extends ConfigClass {
	...
	public function create() {
		...
		$this->q->commit();
		echo json_encode(array("success" => true, "message" => "Record Created", "ordersstatusId" => $ordersstatusId));
		exit();
	}
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	AND `ordersstatus`.`isActive`		=	1	";
			} else if ($this->q->vendor == self::MSSQL) {
				$this->auditFilter = "	AND [ordersstatus].[isActive]		=	1	";
			} else if ($this->q->vendor == self::ORACLE) {
				$this->auditFilter = "	AND ORDERSTATUS.ISACTIVE	=	1	";
	...
	function update() {
		...
		if ($total == 0) {
			...
			$this->q->tableName = $this->model->getTableName();
			$this->q->primaryKeyName = $this->model->getPrimaryKeyName();
			$this->q->primaryKeyValue = $this->model->getOrdersStatusId(0, 'single');
			$this->q->audit = $this->audit;
			$this->q->update($sql);
			if ($this->q->execute == 'fail') {
				echo json_encode(array("success" => false, "message" => $this->q->responce));
