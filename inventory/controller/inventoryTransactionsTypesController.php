...
class InventoryTransactionsTypesClass extends ConfigClass {
	...
	function duplicate() {
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "inventoryTransactionsTypesDesc" => $row ['inventoryTransactionsTypesDesc']));
			exit();
		} else {
