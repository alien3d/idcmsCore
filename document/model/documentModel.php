...
class DocumentModel extends ValidationClass
{
    ...
    function execute ()
    {
        ...
        if (isset($_POST['documentCategoryId'])) {
            ...
            $this->strict($_POST['documentCategoryId'], 'numeric'));
        }
        ...
        if (isset($_POST['documentSequence'])) {
            ...
            $this->strict($_POST['docuementSequence'], 'numeric'));
        }
        if (isset($_POST['documentCode'])) {
            $this->setDocumentCode(
            $this->strict($_POST['documentCode'], 'memo'));
        }
        if (isset($_POST['documentNote'])) {
            $this->setDocumentNote(
            $this->strict($_POST['documentNote'], 'memo'));
        }
        if (isset($_POST['documentTitle'])) {
            $this->setDocumentTitle(
            $this->strict($_POST['documentTitle'], 'memo'));
        }
        if (isset($_POST['documentDesc'])) {
            $this->setDocumentDesc(
            $this->strict($_POST['documentDesc'], 'memo'));
        }
        if (isset($_POST['documentPath'])) {
            $this->setDocumentPath(
            $this->strict($_POST['documentPath'], 'memo'));
        }
        ...
        for ($i = 0; $i < $this->getTotal(); $i ++) {
            if (isset($_GET['documentId'])) {
                $this->setDocumentId(
                $this->strict($_GET['documentId'][$i], 'numeric'), $i, 'array');
            }
    ...
    public function setDocumentId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:setDocumentId ?"));
                exit();
            }
    }
    /**
    ...
    public function getDocumentId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:getDocumentId ?"));
                exit();
            }
    }
    /**
