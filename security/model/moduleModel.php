...
class ModuleModel extends ValidationClass
{
    ...
    function execute ()
    {
        ...
        if (isset($_POST['moduleId'])) {
            $this->setModuleId($this->strict($_POST['moduleId'], 'numeric'), 0, 
            'single');
        }
        ...
        if (isset($_POST['moduleSequence'])) {
            $this->setModuleSequence(
            $this->strict($_POST['moduleSequence'], 'numeric'));
        }
        ...
        for ($i = 0; $i < $this->getTotal(); $i ++) {
            $this->setModuleId($this->strict($_GET['moduleId'][$i], 'numeric'), 
            $i, 'array');
    ...
    public function setModuleId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:setModuleId ?"));
                exit();
            }
    }
    /**
    ...
    public function getModuleId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:getModuleId ?"));
                exit();
            }
    }
    /**
