...
class moduleAccessModel extends validationClass
{
    ...
    function execute ()
    {
        ...
        for ($i = 0; $i < $this->getTotal(); $i ++) {
            ...
            $this->strict($_GET['moduleAccessId'][$i], 'numeric'), $i, 'array');
    ...
    public function setModuleAccessId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:setModuleAccessId ?"));
                exit();
            }
    }
    /**
    ...
    public function getModuleAccessId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:getModuleAccessId ?"));
                exit();
            }
    }
    /**
