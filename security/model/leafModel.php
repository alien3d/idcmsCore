...
class LeafModel extends ValidationClass
{
    ...
    function execute ()
    {
        ...
        if (isset($_POST['leafId'])) {
            $this->setLeafId($this->strict($_POST['leafId'], 'numeric'), 0, 
            'single');
        }
        ...
        if (isset($_POST['leafSequence'])) {
            $this->setLeafSequence(
            $this->strict($_POST['leafSequence'], 'numeric'));
        }
        if (isset($_POST['leafFilename'])) {
            $this->setLeafFilename($this->strict($_POST['leafFilename'], 
            'memo'));
        }
        ...
        for ($i = 0; $i < $this->getTotal(); $i ++) {
            $this->setLeafId($this->strict($_GET['leafId'][$i], 'numeric'), $i, 
            'array');
    ...
    public function setLeafId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:setLeafId ?"));
                exit();
            }
    }
    /**
    ...
    public function getLeafId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type String Or Array:getLeafId ?"));
                exit();
            }
    }
    /**
