...
class LeafTranslateClass extends ConfigClass
{
    ...
    function create ()
    {
        ...
        if ($this->getVendor() == self::mysql) {
			...
			VALUES
					(
						\"" .
                     $this->model->accordionId . "\",	\"" .
                     $this->model->folderId . "\",
						\"" .
                     $this->model->leafNote . "\",		\"" .
                     $this->model->leafSequence . "\",
						\"" .
                     $this->model->leafCode . "\",		\"" .
                     $this->model->leafFilename .
                     "\",
						\"" .
                     $this->model->iconId . "\",			\"" .
                     $this->model->getIsNew(0, 'single') .
        ...
        $this->q->commit();
        echo json_encode(
        array("success" => true, "leafId" => $lastId, 
    ...
    function read ()
    {
         ...
         */
        $tableArray = array('accordion', 'accordionTranslate', 'folder', 
    ...
    function translateRead ()
    {
        ...
        if ($this->q->vendor = 'mysql') {
			...
			JOIN 	`language`
			USING (`languageId`)
			WHERE	`leafTranslate`.`leafId`=\"" .
             $this->strict($_POST['leafId'], 'numeric') . "\"";
        } else 
            if ($this->getVendor() == self::mssql) {
                $sql = "
			SELECT	*
    ...
    public function translateUpdate ()
    {
		...
		SET		`leafTranslate` 	=	\"" .
             $this->strict($_POST['leafTranslate'], 'string') .
             "\"
		WHERE 	`leafTranslateId`	=	\"" .
             $this->strict($_POST['TranslateId'], 'numeric') . "\"";
        } else 
            if ($this->getVendor() == self::mssql) {
                $sql = "
		UPDATE	[leafTranslate]
		...
		SET		\"leafTranslate\" 		=	\"" .
                     ...
                     "\"
		WHERE 	\"leafTranslateId\"	=	\"" .
                     $this->strict($_POST['accordionTranslateId'], 'numeric') .
                     "\"";
                }
        $this->q->update($sql);
        if ($this->q->execute == 'fail') {
...
if (isset($_POST['method'])) {
    ...
    if (isset($_POST['leafId_temp'])) {
        $leafTranslateObject->leafId_temp = $_POST['leafId_temp'];
    }
    /*
	 *  Leaf / Application Identification
	 */
    if (isset($_POST['leafId'])) {
        $leafTranslateObject->leafId = $_POST['leafId'];
    }
    /*
	 * Admin Only
	 */
...
if (isset($_GET['method'])) {
    ...
    if (isset($_GET['leafId_temp'])) {
        $leafTranslateObject->leafId_temp = $_GET['leafId_temp'];
    }
    ...
    if (isset($_GET['field'])) {
        if ($_GET['field'] == 'staffId') {
            $leafTranslateObject->staffId();
        }
        if ($_GET['field'] == 'accordionId') {
            $leafTranslateObject->accordion();
        }
        if ($_GET['field'] == 'folderId') {
            $leafTranslateObject->folder();
        }
        if ($_GET['field'] == 'sequence') {
            $leafTranslateObject->nextSequence();
        }
    }
    if (isset($_GET['mode'])) {
        if ($_GET['mode'] == 'excel') {
    ...
    if ($_GET['method'] == 'translate') {
        $leafTranslateObject->translateMe();
    }
}
?>
