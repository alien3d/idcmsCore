...
class tabClass extends configClass
{
    ...
    public $model;
    /**
     * Accordion Translate Identification
     * @var  numeric $accordionTranslateId
     */
    private $tabTranslateId;
    /**
     * Accordion Translate
     * @var string $accordionTranslate
     */
    public $tabTranslate;
    /**
     * Class Loader
    ...
    function create()
    {
        ...
        if ($this->q->vendor == self::mysql) {
			...
			VALUES
					(
						'" . $this->model->iconId . "',			'" . $this->model->tabSequence . "',
						'" . $this->model->tabNote . "',	'" . $this->model->isNew . "',
    ...
    function read()
    {
        ...
        if ($this->q->vendor == self::mysql) {
			...
			AND			`icon`.`isActive`		=	1 ";
            if (($this->tabId)) {
                $sql .= " AND `tabId`='" . $this->strict($this->tabId, 'numeric') . "'";
            }
        } else if ($this->q->vendor == self::mssql) {
            $sql = "
			SELECT		*
