Ext.onReady(function() {
    ...
    var leafProxy = new Ext.data.HttpProxy({
        ...
        method: 'POST',
        success: function(response, options) {
            jsonResponse = Ext.decode(response.responseText); // Ext.MessageBox.alert(systemLabel, jsonResponse.message);
        },
        failure: function(response, options) {
            Ext.MessageBox.alert(systemErrorLabel, escape(response.Status) + ':' + escape(response.statusText));
        }
    ...
    var gridPanel = new Ext.Panel({
        ...
        '-', {
            ...
            handler: function() {
                Ext.Ajax.request({
                    url: '../controller/leafController.php?method=report&mode=excel&limit=' + perPage + '&leafId=' + leafId,
                    method: 'GET',
                    success: function(response, options) {
                        jsonResponse = Ext.decode(response.responseText);
                        if (jsonResponse.success == true) {
                            window.open('../security/document/excel/leaf.xlsx');
                        } else {
                            Ext.MessageBox.alert(systemErrorLabel, jsonResponse.message);
                        }
                    },
