...
class DefaultLabelModel extends ValidationClass
{
	...
	function execute ()
	{
		...
		if (isset($_POST['defaultLabelId'])) {
			$this->setdefaultLabelId(
			$this->strict($_POST['defaultLabelId'], 'numeric'), 0, 'single');
		}
		if (isset($_POST['defaultLabel'])) {
			$this->setdefaultLabel($this->strict($_POST['defaultLabel'],
            'memo'));
		}
		if (isset($_POST['defaultLabelEnglish'])) {
			$this->setdefaultLabelEnglish(
			$this->strict($_POST['defaultLabelEnglish'], 'memo'));
		}
		if (isset($_POST['defaultLabelSequence'])) {
			$this->setdefaultLabelSequence(
			$this->strict($_POST['defaultLabelSequence'], 'memo'));
		}
		if (isset($_POST['defaultLabelCode'])) {
			$this->setdefaultLabelCode(
			$this->strict($_POST['defaultLabelCode'], 'memo'));
		}
		if (isset($_POST['defaultLabelNote'])) {
			$this->setdefaultLabelNote(
			$this->strict($_POST['defaultLabelNote'], 'memo'));
		}
		...
		for ($i = 0; $i < $this->getTotal(); $i ++) {
			$this->setDefaultLabelId(
			$this->strict($_GET['defaultLabelId'][$i], 'numeric'), $i, 'array');
	...
	public function setDefaultLabelId ($value, $key, $type)
	{
		...
		if ($type == 'array') {
			...
			echo json_encode(
			array("success" => false,
                "message" => "Cannot Identifiy Type Single Or Array:setDefaultLabelId ?"));
			exit();
		}
	}
	/**
	...
	public function getDefaultLabelId ($key, $type)
	{
		...
		if ($type == 'array') {
			...
			echo json_encode(
			array("success" => false,
                "message" => "Cannot Identifiy Type Single Or Array:getDefaultLabelId ?"));
			exit();
		}
	}
	/**
