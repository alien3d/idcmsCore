...
class ExtLabelModel extends ValidationClass
{
    ...
    function execute ()
    {
        ...
        if (isset($_POST['extLabelId'])) {
            $this->setextLabelId($this->strict($_POST['extLabelId'], 'numeric'), 
            0, 'single');
        }
        ...
        if (isset($_POST['extLabelCode'])) {
            $this->setextLabelCode($this->strict($_POST['extLabelCode'], 
            'memo'));
        }
        if (isset($_POST['extLabelNote'])) {
            $this->setextLabelNote($this->strict($_POST['extLabelNote'], 
            'memo'));
        }
    ...
    public function setExtLabelId ($value, $key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type Single Or Array:setExtLabelId ?"));
                exit();
            }
    }
    /**
    ...
    public function getExtLabelId ($key, $type)
    {
        if ($type == 'single') {
            ...
            if ($type == 'array') {
                ...
                echo json_encode(
                array("success" => false, 
                "message" => "Cannot Identifiy Type Single Or Array: getExtLabelId ?"));
                exit();
            }
    }
    /**
