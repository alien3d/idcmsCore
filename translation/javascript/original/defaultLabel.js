                ...
                listeners: {
                    'click': function(button,e) {
                        ...
                        for (var i = 0; i < modified.length; i++) {
                            ...
                            var record = defaultLabelStore.getAt(i);
                            sub_url = sub_url + '&defaultLabelId[]=' + record.get('defaultLabelId');
                            if (isAdmin == 1) {
    ...
    var defaultLabelTranslateEditor = new Ext.ux.grid.RowEditor({
        ...
        listeners: {
            ...
            afteredit: function(rowEditor, changes, record, rowIndex) {
                ...
                if (parseInt(record.get('defaultLabelId')) == 'NaN') {
                    method = 'create';
                } else if (record.get('defaultLabelId') == '') {
                    method = 'create';
                } else if (record.get('defaultLabelId') == undefined) {
                    method = 'create';
                } else if (record.get('defaultLabelId') > 0) {
                    method = 'save';
                } else {
                    method = 'create';
                }
                Ext.Ajax.request({
                    ...
                    params: {
                        ...
                        defaultLabelNative: Ext.getCmp('defaultLabelNative').getValue()
                    },
                    success: function(response, options) {
                        jsonResponse = Ext.decode(response.responseText);
            ...
            items: [{
                ...
                listeners: {
                    'click': function (button, e) {
                        ...
                        for (var i = 0; i < modified.length; i++) {
                            var dataChanges = modified[i].getChanges();
                            var record = defaultLabelTranslateStore.getAt(i);
                            sub_url = sub_url + '&defaultLabelTranslateId[]=' + record.get('moduleTranslateId');
                            if (isAdmin == 1) {
                                if (dataChanges.isDefault == true || dataChanges.isDefault == false) {
                                    sub_url = sub_url + '&isDefault[]=' + record.get('isDefault');
                                }
