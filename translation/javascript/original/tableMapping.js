Ext.onReady(function() {
    ...
    var tableMappingColumnModel = [new Ext.grid.RowNumberer(), {
        id: 'action',
        header: 'Task',
        xtype: 'actioncolumn',
        width: 50,
                    ...
                    fn: function(response) {
                        if ('yes' == response) {
                            Ext.Ajax.request({
                                ...
                                success: function(response, options) {
                                    jsonResponse = Ext.decode(response.responseText);
                                    if (jsonResponse.success == true) {} else {}
        ...
        listeners: {
            'rowclick': function(object, rowIndex, e) {
                ...
                formPanel.form.load({
                    ...
                    success: function(form, action) {
                        Ext.getCmp('tableMappingDesc_temp').setValue(record.data.tableMappingDesc);
                        viewPort.items.get(1).expand();
                    },
                    failure: function(form, action) {
                        Ext.MessageBox.alert(systemErrorLabel, action.result.message);
            ...
            },
            {
                ...
                listeners: {
                    'click': function(button,e) {
                        ...
                        var sub_url = '';
                        var modified = tableMappingStore.getModifiedRecords();
                        for (var i = 0; i < modified.length; i++) {
                            var dataChanges = modified[0].getChanges();
                            var record = tableMappingStore.getAt(i);
                            sub_url = sub_url + '&tableMappingId[]=' + record.get('tableMappingId');
                            if (isAdmin == 1) {
            ...
            afteredit: function(rowEditor, changes, record, rowIndex) {
                ...
                Ext.Ajax.request({
                    ...
                    params: {
                        ...
                        isAdmin: isAdmin,
                        method: method,
                        tableMappingTranslateId: record.get('tableMappingTranslateId'),
                        tableMappingTranslate: Ext.getCmp('tableMappingTranslate').getValue()
                    },
                    success: function(response, options) {
                        jsonResponse = Ext.decode(response.responseText);
    ...
    var gridPanel = new Ext.Panel({
        ...
        '-', {
            ...
            handler: function() {
                Ext.Ajax.request({
                    url: '../controller/tableMappingController.php?method=report&mode=excel&limit=' + perPage + '&leafId=' + leafId,
                    method: 'GET',
                    success: function(response, options) {
                        jsonResponse = Ext.decode(response.responseText);
                        if (jsonResponse == true) {
                            window.open('../../security/document/excel/tableMapping.xlsx');
                        } else {
            ...
            handler: function() {
                ...
                formPanel.getForm().submit({
                    ...
                    success: function(form, action) {
                        ...
                        Ext.getCmp('translation').enable();
                        Ext.getCmp('tableMappingId').setValue(action.result.tableMappingId);
                    },
                    failure: function(form, action) {
            ...
            handler: function() {
                Ext.Ajax.request({
                    ...
                    params: {
                        leafId: leafId,
                        method: 'translate',
                        tableMappingId: Ext.getCmp('tableMappingId').getValue()
                    },
                    success: function(response, options) {
                        jsonResponse = Ext.decode(response.responseText);
                        if (jsonResponse.success == true) {
