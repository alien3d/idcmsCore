                ...
                listeners: {
                    'click': function(button,e) {
                        ...
                        for (var i = 0; i < modified.length; i++) {
                            ...
                            var record = systemStringStore.getAt(i);
                            sub_url = sub_url + '&systemStringId[]=' + record.get('systemStringId');
                            if (isAdmin == 1) {
    ...
    var systemStringTranslateEditor = new Ext.ux.grid.RowEditor({
        ...
        listeners: {
            ...
            afteredit: function(rowEditor, changes, record, rowIndex) {
                ...
                if (parseInt(record.get('systemStringId')) == 'NaN') {
                    method = 'create';
                } else if (record.get('systemStringId') == '') {
                    method = 'create';
                } else if (record.get('systemStringId') == undefined) {
                    method = 'create';
                } else if (record.get('systemStringId') > 0) {
                    method = 'save';
                } else {
                    method = 'create';
                }
                Ext.Ajax.request({
                    ...
                    params: {
                        ...
                        systemStringNative: Ext.getCmp('systemStringNative').getValue()
                    },
                    success: function(response, options) {
                        jsonResponse = Ext.decode(response.responseText);
