...
require_once ("../model/lateInterestTypeModel.php");

/**
 * this is lateInterestType setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author hafizan
...
class LateInterestTypeClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`lateInterestType`
					(
			...
			VALUES
					(
						'" . $this->model->getLateInterestTypeSequence() . "',
						'" . $this->model->getLateInterestTypeCode() . "',
						'" . $this->model->getLateInterestTypeDesc() . "',
		...
		$this->q->create($sql);
		$lateInterestTypeId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "  `lateInterestType`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			SELECT		GENERALLEDGERCHARTOFACCOUNTTYPE.GENERALLEDGERCHARTOFACCOUNTTYPEID   		 	AS 	\"lateInterestTypeId\",
						GENERALLEDGERCHARTOFACCOUNTTYPE.GENERALLEDGERCHARTOFACCOUNTTYPESEQUENCE 				AS 	\"lateInterestTypeSequence\",
						GENERALLEDGERCHARTOFACCOUNTTYPE.GENERALLEDGERCHARTOFACCOUNTTYPECODE 			AS 	\"lateInterestTypeCode\",
						GENERALLEDGERCHARTOFACCOUNTTYPE.GENERALLEDGERCHARTOFACCOUNTTYPEDESC 			AS 	\"lateInterestTypeDesc\",
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getLateInterestTypeId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getLateInterestTypeId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('lateInterestTypeId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('lateInterestType');
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getLateInterestTypeId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			WHERE 		['".$this->q->getFinancialDatabase()."'].[lateInterestType].[lateInterestTypeId]			=	'" . $this->model->getLateInterestTypeId(0, 'single') . "'";
				...
				SET 		GENERALLEDGERCHARTOFACCOUNTTYPESEQUENCE	=	'" . $this->model->getLateInterestTypeSequence() . "',
							GENERALLEDGERCHARTOFACCOUNTTYPECODE		=	'" . $this->model->getLateInterestTypeCode() . "',
							GENERALLEDGERCHARTOFACCOUNTTYPEDESC		=	'" . $this->model->getLateInterestTypeDesc() . "',
			...
			WHERE 		GENERALLEDGERCHARTOFACCOUNTTYPEID		=	'" . $this->model->getLateInterestTypeId(0, 'single') . "'";
			} else if ($this->getVendor() == self::DB2) {
				$sql = "
			UPDATE			GENERALLEDGERCHARTOFACCOUNTTYPE
			SET 			GENERALLEDGERCHARTOFACCOUNTTYPESEQUENCE	=	'" . $this->model->getLateInterestTypeSequence() . "',
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getLateInterestTypeId($i, 'array') . "'
	...
	function duplicate() {
		...
		if ($this->getVendor() == self::MYSQL) {
			$sql = "
			SELECT	`lateInterestTypeSequence`
			FROM 	`lateInterestType`
			WHERE 	`lateInterestTypeSequence` 	= 	'" . $this->model->getLateInterestTypeSequence() . "'
			AND		`isActive`		=	1";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[lateInterestTypeSequence]
			...
			FROM 	GENERALLEDGERCHARTOFACCOUNTTYPE
			WHERE 	GENERALLEDGERCHARTOFACCOUNTTYPESEQUENCE 	= 	'" . $this->model->getLateInterestTypeSequence() . "'
			AND		ISACTIVE		=	1";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	GENERALLEDGERCHARTOFACCOUNTTYPESEQUENCE
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "lateInterestTypeDesc" => $row ['lateInterestTypeDesc']));
			exit();
		} else {
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['lateInterestTypeDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "lateInterestType" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$lateInterestTypeObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$lateInterestTypeObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$lateInterestTypeObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$lateInterestTypeObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$lateInterestTypeObject->setGridQuery($_POST ['filter']);
	}
	...
	if (isset($_POST ['order'])) {
		$lateInterestTypeObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$lateInterestTypeObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$lateInterestTypeObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['lateInterestTypeDesc'])) {
		if (strlen($_GET ['lateInterestTypeDesc']) > 0) {
