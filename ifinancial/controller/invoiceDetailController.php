...
require_once ("../model/invoiceDetailModel.php");
...
class InvoiceDetailClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`invoiceDetail`
					(
						`invoiceId`,												
			...
			VALUES
					(
						...
						'" . $this->model->getInvoiceDetailAmount() . "',
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`invoiceDetail`.`isActive`		=	1	";
			} else if ($this->q->vendor == self::MSSQL) {
				$this->auditFilter = "	['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isActive]		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`invoiceDetail`.`invoiceDetailId`,
					...
					`invoiceDetail`.`invoiceDetailAmount`,
			...
			if ($this->model->getInvoiceDetailId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getInvoiceDetailId(0, 'single') . "'";
			}
			if ($this->model->getInvoiceId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`invoiceId`='" . $this->model->getInvoiceId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[invoiceDetail].[invoiceDetailId],
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getInvoiceId(0, 'single')) {
				$sql .= " AND ['".$this->q->getFinancialDatabase()."'].[" . $this->model->getTableName() . "].[" . $this->model->getPrimaryKeyName() . "]='" . $this->model->getInvoiceId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::ORACLE) {
			$sql = "
			SELECT		INVOICEDETAILID   		 	AS 	\"invoiceDetailId\",
						INVOICEID 				AS 	\"invoiceId\",
						GENERALLEDGERCHARTOFACCOUNTID 			AS 	\"generalLedgerChartOfAccountId\",
						INVOICEDETAILAMOUNT 			AS 	\"gAmount\",
		 ...
		 */
		$filterArray = null;
		$filterArray = array('invoiceDetailId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('invoiceDetail');
		...
		if ($this->getStart() && $this->getLimit()) {
			...
			if ($this->getVendor() == self::MYSQL) {
				 ...
				 */
				$sql = "
							WITH [invoiceDetailDerived] AS
							(
								SELECT 		['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[invoiceDetailId],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[invoiceId],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[generalLedgerChartOfAccountId],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[gAmount],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isDefault],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isNew],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isDraft],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isUpdate],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isDelete],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isApproved],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isReview],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[isPost],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[executeBy],
											['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[executeTime],
											[iManagement].[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY ['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[invoiceDetailId]) AS 'RowNumber'
								FROM 	['".$this->q->getFinancialDatabase()."'].[invoiceDetail]
								JOIN		[iManagement].[staff]
								ON		['".$this->q->getFinancialDatabase()."'].[invoiceDetail].[executeBy] = [iManagement].[staff].[staffId]
								WHERE " . $this->auditFilter . $tempSql . $tempSql2 . "
							)
							SELECT		*
							FROM 		[invoiceDetailDerived]
							WHERE 		[RowNumber]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getInvoiceDetailId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				...
				SET 		`invoiceId`			=	'" . $this->model->getInvoiceId() . "',
							...
							`transactionMode`					=	'".$this->model->getTransactionMode()."',
							`invoiceDetailAmount`	=	'" . $this->model->getInvoiceDetailAmount() . "',
	...
	function delete() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	[" . $this->model->getPrimaryKeyName() . "]
			FROM 	[".$this->q->getFinancialDatabase()."].[" . $this->model->getTableName() . "]
			WHERE  	[" . $this->model->getPrimaryKeyName() . "] = '" . getInvoiceDetailId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::ORACLE) {
			$sql = "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				...
				SET 	`isDefault`			=	'" . $this->model->getIsDefault(0, 'single') . "',
						...
						`executeBy`			=	'" . $this->model->getExecuteBy() . "',
						`executeTime`		=	" . $this->model->getExecuteTime() . "
				WHERE 	`invoiceDetailId`		=	'" . $this->model->getInvoiceId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 	[".$this->q->getFinancialDatabase()."].[invoiceDetail]
				SET 	[invoiceDetail].[isDefault]			=	'" . $this->model->getIsDefault(0, 'single') . "',
						[invoiceDetail].[isNew]				=	'" . $this->model->getIsNew(0, 'single') . "',
						[invoiceDetail].[isDraft]			=	'" . $this->model->getIsDraft(0, 'single') . "',
						[invoiceDetail].[isUpdate]			=	'" . $this->model->getIsUpdate(0, 'single') . "',
						[invoiceDetail].[isDelete]			=	'" . $this->model->getIsDelete(0, 'single') . "',
						[invoiceDetail].[isActive]			=	'" . $this->model->getIsActive(0, 'single') . "',
						[invoiceDetail].[isApproved]		=	'" . $this->model->getIsApproved(0, 'single') . "',
						[invoiceDetail].[isReview]			=	'" . $this->model->getIsReview(0, 'single') . "',
						[invoiceDetail].[isPost]			=	'" . $this->model->getIsPost(0, 'single') . "',
						[invoiceDetail].[executeBy]			=	'" . $this->model->getExecuteBy() . "',
						[invoiceDetail].[executeTime]		=	" . $this->model->getExecuteTime() . "
				WHERE 	[invoiceDetail].[invoiceDetailId]		=	'" .getInvoiceDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::ORACLE) {
				$sql = "
				UPDATE INVOICEDETAIL
				SET 	ISDEFAULT		=	'" . $this->model->getIsDefault(0, 'single') . "',
						...
						EXECUTEBY		=	'" . $this->model->getExecuteBy() . "',
						EXECUTETIME		=	" . $this->model->getExecuteTime() . "
				WHERE 	INVOICEDETAILID		=	'" . getInvoiceDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::DB2) {
				$sql = "
				UPDATE INVOICEDETAIL
				SET 	ISDEFAULT		=	'" . $this->model->getIsDefault(0, 'single') . "',
	...
	function duplicate() {
		...
		/**
		 ...
		 if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`invoiceId`
			FROM 	`invoiceDetail`
			WHERE 	`invoiceId` 	= 	'" . $this->model->getInvoiceId() . "'
			AND		`isActive`		=	1";
			} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[invoiceId]
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "invoiceDetailDesc" => $row ['invoiceDetailDesc']));
			exit();
			} else {
	...
	function trialBalanceChecking(){
		...
		AND   	`invoiceDetail`.`invoiceId`='".$this->model->getInvoiceId()."' ";
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['invoiceDetailDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "invoiceDetail" . rand(0, 10000000) . ".xlsx";
...
$invoiceDetailObject = new GeneralLedgerJournalDetailClass ();
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$invoiceDetailObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$invoiceDetailObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$invoiceDetailObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$invoiceDetailObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$invoiceDetailObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$invoiceDetailObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$invoiceDetailObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$invoiceDetailObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$invoiceDetailObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$invoiceDetailObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$invoiceDetailObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$invoiceDetailObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['invoiceDetailDesc'])) {
		if (strlen($_GET ['invoiceDetailDesc']) > 0) {
