...
require_once ("../model/voucherDetailModel.php");
...
class VoucherDetailClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`voucherDetail`
					(
						`voucherId`,												
		...
		$this->q->create($sql);
		$voucherDetailId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`voucherDetail`.`isActive`		=	1	";
			} else if ($this->q->vendor == self::MSSQL) {
				$this->auditFilter = "	['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isActive]		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`voucherDetail`.`voucherDetailId`,
					...
					`voucherDetail`.`voucherDetailAmount`,
            ...
            ON      `voucherDetail`.`executeBy` = `staff`.`staffId`
			...
			if ($this->model->getVoucherId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`voucherId`='" . $this->model->getVoucherId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[voucherDetail].[voucherDetailId],
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getVoucherId(0, 'single')) {
				$sql .= " AND ['".$this->q->getFinancialDatabase()."'].[" . $this->model->getTableName() . "].[" . $this->model->getPrimaryKeyName() . "]='" . $this->model->getVoucherId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::ORACLE) {
			$sql = "
			SELECT		VOUCHERDETAILID   		 	AS 	\"voucherDetailId\",
						VOUCHERID 				AS 	\"voucherId\",
						GENERALLEDGERCHARTOFACCOUNTID 			AS 	\"generalLedgerChartOfAccountId\",
						VOUCHERDETAILAMOUNT 			AS 	\"gAmount\",
		 ...
		 */
		$filterArray = null;
		$filterArray = array('voucherDetailId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('voucherDetail');
		...
		if ($this->getStart() && $this->getLimit()) {
			...
			if ($this->getVendor() == self::MYSQL) {
				 ...
				 */
				$sql = "
							WITH [voucherDetailDerived] AS
							(
								SELECT 		['".$this->q->getFinancialDatabase()."'].[voucherDetail].[voucherDetailId],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[voucherId],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[generalLedgerChartOfAccountId],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[gAmount],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isDefault],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isNew],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isDraft],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isUpdate],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isDelete],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isApproved],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isReview],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[isPost],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[executeBy],
											['".$this->q->getFinancialDatabase()."'].[voucherDetail].[executeTime],
											[iManagement].[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY ['".$this->q->getFinancialDatabase()."'].[voucherDetail].[voucherDetailId]) AS 'RowNumber'
								FROM 	['".$this->q->getFinancialDatabase()."'].[voucherDetail]
								JOIN		[iManagement].[staff]
								ON		['".$this->q->getFinancialDatabase()."'].[voucherDetail].[executeBy] = [iManagement].[staff].[staffId]
								WHERE " . $this->auditFilter . $tempSql . $tempSql2 . "
							)
							SELECT		*
							FROM 		[voucherDetailDerived]
							WHERE 		[RowNumber]
	...
	function update() {
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				...
				SET 		`voucherId`			=	'" . $this->model->getVoucherId() . "',
							...
							`transactionMode`					=	'".$this->model->getTransactionMode()."',
							`voucherDetailAmount`	=	'" . $this->model->getVoucherDetailAmount() . "',
				...
				WHERE 		`voucherDetailId`		=	'" . $this->model->getVoucherDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 		[".$this->q->getFinancialDatabase()."].[voucherDetail]
				SET 		[voucherId]			=	'" . $this->model->getVoucherId() . "',
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				...
				SET 	`isDefault`			=	'" . $this->model->getIsDefault(0, 'single') . "',
						...
						`executeBy`			=	'" . $this->model->getExecuteBy() . "',
						`executeTime`		=	" . $this->model->getExecuteTime() . "
				WHERE 	`voucherDetailId`		=	'" . $this->model->getVoucherId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 	[".$this->q->getFinancialDatabase()."].[voucherDetail]
				SET 	[voucherDetail].[isDefault]			=	'" . $this->model->getIsDefault(0, 'single') . "',
						[voucherDetail].[isNew]				=	'" . $this->model->getIsNew(0, 'single') . "',
						[voucherDetail].[isDraft]			=	'" . $this->model->getIsDraft(0, 'single') . "',
						[voucherDetail].[isUpdate]			=	'" . $this->model->getIsUpdate(0, 'single') . "',
						[voucherDetail].[isDelete]			=	'" . $this->model->getIsDelete(0, 'single') . "',
						[voucherDetail].[isActive]			=	'" . $this->model->getIsActive(0, 'single') . "',
						[voucherDetail].[isApproved]		=	'" . $this->model->getIsApproved(0, 'single') . "',
						[voucherDetail].[isReview]			=	'" . $this->model->getIsReview(0, 'single') . "',
						[voucherDetail].[isPost]			=	'" . $this->model->getIsPost(0, 'single') . "',
						[voucherDetail].[executeBy]			=	'" . $this->model->getExecuteBy() . "',
						[voucherDetail].[executeTime]		=	" . $this->model->getExecuteTime() . "
				WHERE 	[voucherDetail].[voucherDetailId]		=	'" .getVoucherDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::ORACLE) {
				$sql = "
				UPDATE VOUCHERDETAIL
				SET 	ISDEFAULT		=	'" . $this->model->getIsDefault(0, 'single') . "',
						...
						EXECUTEBY		=	'" . $this->model->getExecuteBy() . "',
						EXECUTETIME		=	" . $this->model->getExecuteTime() . "
				WHERE 	VOUCHERDETAILID		=	'" . getVoucherDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::DB2) {
				$sql = "
				UPDATE VOUCHERDETAIL
				SET 	ISDEFAULT		=	'" . $this->model->getIsDefault(0, 'single') . "',
	...
	function duplicate() {
		...
		/**
		 ...
		 if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`voucherId`
			FROM 	`voucherDetail`
			WHERE 	`voucherId` 	= 	'" . $this->model->getVoucherId() . "'
			AND		`isActive`		=	1";
			} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[voucherId]
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "voucherDetailDesc" => $row ['voucherDetailDesc']));
			exit();
			} else {
	...
	function trialBalanceChecking(){
		...
		AND   	`voucherDetail`.`voucherId`='".$this->model->getVoucherId()."' ";
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['voucherDetailDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "voucherDetail" . rand(0, 10000000) . ".xlsx";
...
$voucherDetailObject = new GeneralLedgerJournalDetailClass ();
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['leafId'])) {
		$voucherDetailObject->setLeafId($_POST ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_POST ['isAdmin'])) {
		$voucherDetailObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$voucherDetailObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$voucherDetailObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$voucherDetailObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$voucherDetailObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$voucherDetailObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$voucherDetailObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$voucherDetailObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$voucherDetailObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$voucherDetailObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$voucherDetailObject->setSortField($_POST ['sortField']);
	}
	/*
	 *  Load the dynamic value
	 */
	$voucherDetailObject->execute();
	/*
	 *  Crud Operation (Create Read Update Delete/Destory)
	 */
	if ($_POST ['method'] == 'create') {
		$voucherDetailObject->create();
	}
	if ($_POST ['method'] == 'save') {
		$voucherDetailObject->update();
	}
	if ($_POST ['method'] == 'read') {
		$voucherDetailObject->read();
	}
	if ($_POST ['method'] == 'delete') {
		$voucherDetailObject->delete();
	}
}
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['leafId'])) {
		$voucherDetailObject->setLeafId($_GET ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_GET ['isAdmin'])) {
		$voucherDetailObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	...
	if ($_GET ['method'] == 'updateStatus') {
		$voucherDetailObject->updateStatus();
	}
	/*
	 *  Checking Any Duplication  Key
	 */
	if (isset($_GET ['voucherDetailDesc'])) {
		if (strlen($_GET ['voucherDetailDesc']) > 0) {
			$voucherDetailObject->duplicate();
		}
	}
	/**
	 * Button Navigation
	...
	if ($_GET ['method'] == 'dataNavigationRequest') {
		if ($_GET ['dataNavigation'] == 'firstRecord') {
			$voucherDetailObject->firstRecord('json');
		}
		if ($_GET ['dataNavigation'] == 'previousRecord') {
			$voucherDetailObject->previousRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'nextRecord') {
			$voucherDetailObject->nextRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'lastRecord') {
			$voucherDetailObject->lastRecord('json');
		}
	}
	/*
	 * Excel Reporting
