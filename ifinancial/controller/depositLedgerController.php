...
require_once ("../model/depositLedgerModel.php");
 ...
 */
class DepositLedgerClass extends ConfigClass {
	...
	function execute() {
		...
		$this->model = new DepositLedgerModel ();
...
".$this->model->getExecuteTime().");";
		...
		$depositLedgerId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			if ($this->model->getDepositLedgerId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getDepositLedgerId(0, 'single') . "'";
			}
		 ...
		 */
		$filterArray = null;
		$filterArray = array('depositLedgerId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('depositLedger');
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDepositLedgerId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
	...
	function delete() {
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE 	`".$this->q->getFinancialDatabase()."`.`depositLedger`
				SET 	`depositLedger`.`isDefault`			=	'" . $this->model->getIsDefault(0, 'single') . "',
						`depositLedger`.`isNew`				=	'" . $this->model->getIsNew(0, 'single') . "',
						`depositLedger`.`isDraft`			=	'" . $this->model->getIsDraft(0, 'single') . "',
						`depositLedger`.`isUpdate`			=	'" . $this->model->getIsUpdate(0, 'single') . "',
						`depositLedger`.`isDelete`			=	'" . $this->model->getIsDelete(0, 'single') . "',
						`depositLedger`.`isActive`			=	'" . $this->model->getIsActive(0, 'single') . "',
						`depositLedger`.`isApproved`		=	'" . $this->model->getIsApproved(0, 'single') . "',
						`depositLedger`.`isReview`			=	'" . $this->model->getIsReview(0, 'single') . "',
						`depositLedger`.`isPost`			=	'" . $this->model->getIsPost(0, 'single') . "',
						`depositLedger`.`executeBy`			=	'" . $this->model->getExecuteBy() . "',
						`depositLedger`.`executeTime`		=	" . $this->model->getExecuteTime() . "
				WHERE 	`depositLedgerId`		=	'" . $this->model->getDepositLedgerId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 	[".$this->q->getFinancialDatabase()."].[depositLedger]
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getDepositLedgerId($i, 'array') . "'
	...
	function closingChecking(){
		$arrayDate = explode("-",$this->model->getDepositLedgerDate());
	...
	function duplicate() {
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "depositLedgerDesc" => $row ['depositLedgerDesc']));
			exit();
		} else {
	...
	function posting() {
		...
		SELECT 	`depositLedgerDetail`.`depositLedgerDetailId`, 
			  	...
			  	`depositLedgerDetail`.`depositLedgerDetailAmount` AS `generalLedgerLocalAmount`,  
			    ...
			    `depositLedger`.`documentNo`,
			    `depositLedger`.`depositLedgerTitle` AS `generalLedgerTitle`,  
			    `depositLedger`.`depositLedgerDesc` AS `generalLedgerDesc`,
				`depositLedger`.`depositLedgerDate` AS `generalLedgerDate`,
		...
		while($row = $this->q->fetchArray($resultDepositLedgerDetail)) {
			$row['isDefault']=0;
			$row['isNew']=1;
			$row['isDraft']=0;
			$row['isUpdate']=0;
		...
		SET 	`isPost`=1 
		WHERE 	`depositLedgerId`='".$this->model->getDepositLedgerId(0, 'single')."'";
		$this->q->update($sqlDepositLedger);
		if ($this->q->execute == 'fail') {
			echo json_encode(array("success" => false, "message" => $this->q->responce));
			exit();
		}
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['depositLedgerDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "depositLedger" . rand(0, 10000000) . ".xlsx";
...
$depositLedgerObject = new DepositLedgerClass ();
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['leafId'])) {
		$depositLedgerObject->setLeafId($_POST ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_POST ['isAdmin'])) {
		$depositLedgerObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$depositLedgerObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$depositLedgerObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$depositLedgerObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$depositLedgerObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$depositLedgerObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$depositLedgerObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$depositLedgerObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {

		$depositLedgerObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$depositLedgerObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$depositLedgerObject->setSortField($_POST ['sortField']);
	}
	/*
	 *  Load the dynamic value
	 */
	$depositLedgerObject->execute();
	/*
	 *  Crud Operation (Create Read Update Delete/Destory)
	 */
	if ($_POST ['method'] == 'create') {
		$depositLedgerObject->create();
	}
	if ($_POST ['method'] == 'save') {
		$depositLedgerObject->update();
	}
	if ($_POST ['method'] == 'read') {
		$depositLedgerObject->read();
	}
	if ($_POST ['method'] == 'delete') {
		$depositLedgerObject->delete();
	}
	if($_POST['method'] == 'posting'){
		$depositLedgerObject->posting();
	}
}
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['leafId'])) {
		$depositLedgerObject->setLeafId($_GET ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_GET ['isAdmin'])) {
		$depositLedgerObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	...
	if ($_GET ['method'] == 'updateStatus') {
		$depositLedgerObject->updateStatus();
	}
	/*
	 *  Checking Any Duplication  Key
	 */
	if (isset($_GET ['depositLedgerDesc'])) {
		if (strlen($_GET ['depositLedgerDesc']) > 0) {
			$depositLedgerObject->duplicate();
		}
	}
	/**
	 * Button Navigation
	...
	if ($_GET ['method'] == 'dataNavigationRequest') {
		if ($_GET ['dataNavigation'] == 'firstRecord') {
			$depositLedgerObject->firstRecord('json');
		}
		if ($_GET ['dataNavigation'] == 'previousRecord') {
			$depositLedgerObject->previousRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'nextRecord') {
			$depositLedgerObject->nextRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'lastRecord') {
			$depositLedgerObject->lastRecord('json');
		}
	}
	/*
	 * Excel Reporting
