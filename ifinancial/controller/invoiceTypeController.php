...
require_once ("../model/invoiceTypeModel.php");

/**
 * this is invoiceType setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author hafizan
 * @package Account Receivable
 * @subpackage Invoice Type
...
class InvoiceTypeClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`invoiceType`
					(
						`invoiceTypeSequence`,												
						`invoiceTypeCode`,
						`invoiceTypeDesc`,
			...
			VALUES
					(
						'" . $this->model->getInvoiceTypeSequence() . "',
						'" . $this->model->getInvoiceTypeCode() . "',
						'" . $this->model->getInvoiceTypeDesc() . "',
		...
		$this->q->create($sql);
		$invoiceTypeId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "  `invoiceType`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`invoiceType`.`invoiceTypeId`,
						...
						`invoiceType`.`invoiceTypePeriodLimit`,
			...
			if ($this->model->getInvoiceCategoryId()) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`invoiceCategoryId`='" . $this->model->getInvoiceCategoryId() . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[invoiceType].[invoiceTypeId],
			...
			SELECT		INVOICETYPE.INVOICETYPEID   		 				AS 	\"invoiceTypeId\",
							...
							INVOICETYPE.INVOICECREDITLIMIT				AS 	\"invoiceTypeCreditLimit\",
							INVOICETYPE.INVOICETYPELIMIT				AS 	\"invoiceTypePeriodLimit\",
			...
			if ($this->model->getInvoiceCategoryId()) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . ".INVOICECATEGORY='" . $this->model->getInvoiceCategoryId() . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
				$sql = "
				SELECT		INVOICETYPE.INVOICETYPEID   		 				AS 	\"invoiceTypeId\",
		 ...
		 */
		$filterArray = null;
		$filterArray = array('invoiceTypeId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('invoiceType');
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getInvoiceTypeId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			WHERE 		[invoiceType].[invoiceTypeId]			=	'" . $this->model->getInvoiceTypeId(0, 'single') . "'";
				...
				$sql = "
				UPDATE		INVOICETYPE
				SET 		INVOICETYPESEQUENCE	=	'" . $this->model->getInvoiceTypeSequence() . "',
							INVOICETYPECODE		=	'" . $this->model->getInvoiceTypeCode() . "',
							INVOICETYPEDESC		=	'" . $this->model->getInvoiceTypeDesc() . "',
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getInvoiceTypeId($i, 'array') . "'
	...
	function duplicate() {
		...
		if ($this->getVendor() == self::MYSQL) {
			$sql = "
			SELECT	`invoiceTypeSequence`
			FROM 	`invoiceType`
			WHERE 	`invoiceTypeSequence` 	= 	'" . $this->model->getInvoiceTypeSequence() . "'
			AND		`isActive`		=	1";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[invoiceTypeSequence]
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "invoiceTypeDesc" => $row ['invoiceTypeDesc']));
			exit();
		} else {
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['invoiceTypeDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "invoiceType" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$invoiceTypeObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$invoiceTypeObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$invoiceTypeObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$invoiceTypeObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$invoiceTypeObject->setGridQuery($_POST ['filter']);
	}
	...
	if (isset($_POST ['order'])) {
		$invoiceTypeObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$invoiceTypeObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$invoiceTypeObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['invoiceTypeDesc'])) {
		if (strlen($_GET ['invoiceTypeDesc']) > 0) {
