...
require_once ("../model/depositTypeModel.php");

/**
 * this is depositType setting files.This sample template file for master record
 * @name IDCMS
 * @version 2
 * @author hafizan
...
class DepositTypeClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`depositType`
					(
						`depositTypeSequence`,												
						`depositTypeCode`,
						`depositTypeDesc`,
			...
			VALUES
					(
						'" . $this->model->getDepositTypeSequence() . "',
						'" . $this->model->getDepositTypeCode() . "',
						'" . $this->model->getDepositTypeDesc() . "',
		...
		$this->q->create($sql);
		$depositTypeId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "  `depositType`.`isActive`		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT		DEPOSITTYPE.DEPOSITTYPEID   		 	AS 	\"depositTypeId\",
						DEPOSITTYPE.DEPOSITTYPESEQUENCE 				AS 	\"depositTypeSequence\",
						DEPOSITTYPE.DEPOSITTYPECODE 			AS 	\"depositTypeCode\",
						DEPOSITTYPE.DEPOSITTYPEDESC 			AS 	\"depositTypeDesc\",
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getDepositTypeId(0, 'single')) {
				$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getDepositTypeId(0, 'single') . "'";
			}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('depositTypeId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('depositType');
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getDepositTypeId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			WHERE 		['".$this->q->getFinancialDatabase()."'].[depositType].[depositTypeId]			=	'" . $this->model->getDepositTypeId(0, 'single') . "'";
				...
				$sql = "
				UPDATE		DEPOSITTYPE
				SET 		DEPOSITTYPESEQUENCE	=	'" . $this->model->getDepositTypeSequence() . "',
							DEPOSITTYPECODE		=	'" . $this->model->getDepositTypeCode() . "',
							DEPOSITTYPEDESC		=	'" . $this->model->getDepositTypeDesc() . "',
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getDepositTypeId($i, 'array') . "'
	...
	function duplicate() {
		...
		if ($this->getVendor() == self::MYSQL) {
			$sql = "
			SELECT	`depositTypeSequence`
			FROM 	`depositType`
			WHERE 	`depositTypeSequence` 	= 	'" . $this->model->getDepositTypeSequence() . "'
			AND		`isActive`		=	1";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[depositTypeSequence]
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "depositTypeDesc" => $row ['depositTypeDesc']));
			exit();
		} else {
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['depositTypeDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "depositType" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$depositTypeObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$depositTypeObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$depositTypeObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$depositTypeObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$depositTypeObject->setGridQuery($_POST ['filter']);
	}
	...
	if (isset($_POST ['order'])) {
		$depositTypeObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$depositTypeObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$depositTypeObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['depositTypeDesc'])) {
		if (strlen($_GET ['depositTypeDesc']) > 0) {
