...
require_once ("../model/refundTypeModel.php");
 ...
 * @author hafizan
 * @package Refund
 * @subpackage Refund Type
 * @link http://www.idcms.org  
 * @link http://en.wikipedia.org/wiki/Refund
 * @license http://www.gnu.org/copyleft/lesser.html LGPL
 */
class RefundTypeClass extends ConfigClass {
	...
	function execute() {
		...
		$this->model = new RefundTypeModel ();
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`refundType`
					(
						`refundTypeSequence`,												
						`refundTypeCode`,
						`refundTypeDesc`,
			...
			VALUES
					(
						'" . $this->model->getRefundTypeSequence() . "',
						'" . $this->model->getRefundTypeCode() . "',
						'" . $this->model->getRefundTypeDesc() . "',
		...
		$this->q->create($sql);
		$refundTypeId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "  `refundType`.`isActive`		=	1	";
		 ...
		 */
		$filterArray = null;
		$filterArray = array('refundTypeId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('refundType');
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" . $this->model->getRefundTypeId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[" . $this->model->getPrimaryKeyName() . "]
			...
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getRefundTypeId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getRefundTypeId($i, 'array') . "'
	...
	function duplicate() {
		...
		if ($this->getVendor() == self::MYSQL) {
			$sql = "
			SELECT	`refundTypeSequence`
			FROM 	`refundType`
			WHERE 	`refundTypeSequence` 	= 	'" . $this->model->getRefundTypeSequence() . "'
			AND		`isActive`		=	1";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[refundTypeSequence]
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "refundTypeDesc" => $row ['refundTypeDesc']));
			exit();
		} else {
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['refundTypeDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "refundType" . rand(0, 10000000) . ".xlsx";
...
$refundTypeObject = new RefundTypeClass ();
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['leafId'])) {
		$refundTypeObject->setLeafId($_POST ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_POST ['isAdmin'])) {
		$refundTypeObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$refundTypeObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$refundTypeObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$refundTypeObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$refundTypeObject->setGridQuery($_POST ['filter']);
	}
	...
	if (isset($_POST ['order'])) {
		$refundTypeObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$refundTypeObject->setSortField($_POST ['sortField']);
	}
	/*
	 *  Load the dynamic value
	 */
	$refundTypeObject->execute();
	/*
	 *  Crud Operation (Create Read Update Delete/Destory)
	 */
	if ($_POST ['method'] == 'create') {
		$refundTypeObject->create();
	}
	if ($_POST ['method'] == 'save') {
		$refundTypeObject->update();
	}
	if ($_POST ['method'] == 'read') {
		$refundTypeObject->read();
	}
	if ($_POST ['method'] == 'delete') {
		$refundTypeObject->delete();
	}
}
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['leafId'])) {
		$refundTypeObject->setLeafId($_GET ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_GET ['isAdmin'])) {
		$refundTypeObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	...
	if ($_GET ['method'] == 'updateStatus') {
		$refundTypeObject->updateStatus();
	}
	/*
	 *  Checking Any Duplication  Key
	 */
	if (isset($_GET ['refundTypeDesc'])) {
		if (strlen($_GET ['refundTypeDesc']) > 0) {
			$refundTypeObject->duplicate();
		}
	}
	/**
	 * Button Navigation
	...
	if ($_GET ['method'] == 'dataNavigationRequest') {
		if ($_GET ['dataNavigation'] == 'firstRecord') {
			$refundTypeObject->firstRecord('json');
		}
		if ($_GET ['dataNavigation'] == 'previousRecord') {
			$refundTypeObject->previousRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'nextRecord') {
			$refundTypeObject->nextRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'lastRecord') {
			$refundTypeObject->lastRecord('json');
		}
	}
	/*
	 * Excel Reporting
