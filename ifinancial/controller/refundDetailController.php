...
require_once ("../model/refundDetailModel.php");
 ...
 */
class RefundDetailClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql = "
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`refundDetail`
					(
						`refundId`,												
		...
		$this->q->create($sql);
		$refundDetailId = $this->q->lastInsertId();
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "	`refundDetail`.`isActive`		=	1	";
			} else if ($this->q->vendor == self::MSSQL) {
				$this->auditFilter = "	['".$this->q->getFinancialDatabase()."'].[refundDetail].[isActive]		=	1	";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`refundDetail`.`refundDetailId`,
					...
					`refundDetail`.`refundDetailAmount`,
            ...
            ON      `refundDetail`.`executeBy` = `staff`.`staffId`
            WHERE  ". $this->auditFilter;
			if ($this->model->getRefundDetailId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getRefundDetailId(0, 'single') . "'";
			}
			if ($this->model->getRefundId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`refundId`='" . $this->model->getRefundId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[refundDetail].[refundDetailId],
			...
			WHERE 	" . $this->auditFilter;
			if ($this->model->getRefundId(0, 'single')) {
				$sql .= " AND ['".$this->q->getFinancialDatabase()."'].[" . $this->model->getTableName() . "].[" . $this->model->getPrimaryKeyName() . "]='" . $this->model->getRefundId(0, 'single') . "'";
			}
		} else if ($this->getVendor() == self::ORACLE) {
			$sql = "
			SELECT		REFUNDDETAILID   		 	AS 	\"refundDetailId\",
						REFUNDID 				AS 	\"refundId\",
						GENERALLEDGERCHARTOFACCOUNTID 			AS 	\"generalLedgerChartOfAccountId\",
						REFUNDDETAILAMOUNT 			AS 	\"gAmount\",
		 ...
		 */
		$filterArray = null;
		$filterArray = array('refundDetailId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('refundDetail');
		...
		if ($this->getStart() && $this->getLimit()) {
			...
			if ($this->getVendor() == self::MYSQL) {
				 ...
				 */
				$sql = "
							WITH [refundDetailDerived] AS
							(
								SELECT 		['".$this->q->getFinancialDatabase()."'].[refundDetail].[refundDetailId],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[refundId],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[generalLedgerChartOfAccountId],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[gAmount],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isDefault],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isNew],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isDraft],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isUpdate],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isDelete],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isApproved],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isReview],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[isPost],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[executeBy],
											['".$this->q->getFinancialDatabase()."'].[refundDetail].[executeTime],
											[iManagement].[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY ['".$this->q->getFinancialDatabase()."'].[refundDetail].[refundDetailId]) AS 'RowNumber'
								FROM 	['".$this->q->getFinancialDatabase()."'].[refundDetail]
								JOIN		[iManagement].[staff]
								ON		['".$this->q->getFinancialDatabase()."'].[refundDetail].[executeBy] = [iManagement].[staff].[staffId]
								WHERE " . $this->auditFilter . $tempSql . $tempSql2 . "
							)
							SELECT		*
							FROM 		[refundDetailDerived]
							WHERE 		[RowNumber]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			WHERE  	`" . $this->model->getPrimaryKeyName() . "` = '" .$this->model->getRefundDetailId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[" . $this->model->getPrimaryKeyName() . "]
			FROM 	[".$this->q->getFinancialDatabase()."].[" . $this->model->getTableName() . "]
			...
			WHERE  	" . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getRefundDetailId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				...
				SET 		`refundId`			=	'" . $this->model->getRefundId() . "',
							...
							`transactionMode`					=	'".$this->model->getTransactionMode()."',
							`refundDetailAmount`	=	'" . $this->model->getRefundDetailAmount() . "',
				...
				WHERE 		`refundDetailId`		=	'" . $this->model->getRefundDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 		[".$this->q->getFinancialDatabase()."].[refundDetail]
				SET 		[refundId]			=	'" . $this->model->getRefundId() . "',
	...
	function delete() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	[" . $this->model->getPrimaryKeyName() . "]
			FROM 	[".$this->q->getFinancialDatabase()."].[" . $this->model->getTableName() . "]
			WHERE  	[" . $this->model->getPrimaryKeyName() . "] = '" . getRefundDetailId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::ORACLE) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getRefundDetailId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				...
				SET 	`isDefault`			=	'" . $this->model->getIsDefault(0, 'single') . "',
						...
						`executeBy`			=	'" . $this->model->getExecuteBy() . "',
						`executeTime`		=	" . $this->model->getExecuteTime() . "
				WHERE 	`refundDetailId`		=	'" . $this->model->getRefundId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 	[".$this->q->getFinancialDatabase()."].[refundDetail]
				SET 	[refundDetail].[isDefault]			=	'" . $this->model->getIsDefault(0, 'single') . "',
						[refundDetail].[isNew]				=	'" . $this->model->getIsNew(0, 'single') . "',
						[refundDetail].[isDraft]			=	'" . $this->model->getIsDraft(0, 'single') . "',
						[refundDetail].[isUpdate]			=	'" . $this->model->getIsUpdate(0, 'single') . "',
						[refundDetail].[isDelete]			=	'" . $this->model->getIsDelete(0, 'single') . "',
						[refundDetail].[isActive]			=	'" . $this->model->getIsActive(0, 'single') . "',
						[refundDetail].[isApproved]		=	'" . $this->model->getIsApproved(0, 'single') . "',
						[refundDetail].[isReview]			=	'" . $this->model->getIsReview(0, 'single') . "',
						[refundDetail].[isPost]			=	'" . $this->model->getIsPost(0, 'single') . "',
						[refundDetail].[executeBy]			=	'" . $this->model->getExecuteBy() . "',
						[refundDetail].[executeTime]		=	" . $this->model->getExecuteTime() . "
				WHERE 	[refundDetail].[refundDetailId]		=	'" .getRefundDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::ORACLE) {
				$sql = "
				UPDATE REFUNDDETAIL
				SET 	ISDEFAULT		=	'" . $this->model->getIsDefault(0, 'single') . "',
						...
						EXECUTEBY		=	'" . $this->model->getExecuteBy() . "',
						EXECUTETIME		=	" . $this->model->getExecuteTime() . "
				WHERE 	REFUNDDETAILID		=	'" . getRefundDetailId(0, 'single') . "'";
			} else if ($this->getVendor() == self::DB2) {
				$sql = "
				UPDATE REFUNDDETAIL
				SET 	ISDEFAULT		=	'" . $this->model->getIsDefault(0, 'single') . "',
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getRefundDetailId($i, 'array') . "'
	...
	function duplicate() {
		...
		/**
		 ...
		 if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`refundId`
			FROM 	`refundDetail`
			WHERE 	`refundId` 	= 	'" . $this->model->getRefundId() . "'
			AND		`isActive`		=	1";
			} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT	[refundId]
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "refundDetailDesc" => $row ['refundDetailDesc']));
			exit();
			} else {
	...
	function trialBalanceChecking(){
		...
		AND   	`refundDetail`.`refundId`='".$this->model->getRefundId()."' ";
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['refundDetailDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "refundDetail" . rand(0, 10000000) . ".xlsx";
...
$refundDetailObject = new GeneralLedgerJournalDetailClass ();
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['leafId'])) {
		$refundDetailObject->setLeafId($_POST ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_POST ['isAdmin'])) {
		$refundDetailObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$refundDetailObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$refundDetailObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$refundDetailObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$refundDetailObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$refundDetailObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$refundDetailObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$refundDetailObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {
		$refundDetailObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$refundDetailObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$refundDetailObject->setSortField($_POST ['sortField']);
	}
	/*
	 *  Load the dynamic value
	 */
	$refundDetailObject->execute();
	/*
	 *  Crud Operation (Create Read Update Delete/Destory)
	 */
	if ($_POST ['method'] == 'create') {
		$refundDetailObject->create();
	}
	if ($_POST ['method'] == 'save') {
		$refundDetailObject->update();
	}
	if ($_POST ['method'] == 'read') {
		$refundDetailObject->read();
	}
	if ($_POST ['method'] == 'delete') {
		$refundDetailObject->delete();
	}
}
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['leafId'])) {
		$refundDetailObject->setLeafId($_GET ['leafId']);
	}
	/*
	 * Admin Only
	 */
	if (isset($_GET ['isAdmin'])) {
		$refundDetailObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	...
	if ($_GET ['method'] == 'updateStatus') {
		$refundDetailObject->updateStatus();
	}
	/*
	 *  Checking Any Duplication  Key
	 */
	if (isset($_GET ['refundDetailDesc'])) {
		if (strlen($_GET ['refundDetailDesc']) > 0) {
			$refundDetailObject->duplicate();
		}
	}
	/**
	 * Button Navigation
	...
	if ($_GET ['method'] == 'dataNavigationRequest') {
		if ($_GET ['dataNavigation'] == 'firstRecord') {
			$refundDetailObject->firstRecord('json');
		}
		if ($_GET ['dataNavigation'] == 'previousRecord') {
			$refundDetailObject->previousRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'nextRecord') {
			$refundDetailObject->nextRecord('json', 0);
		}
		if ($_GET ['dataNavigation'] == 'lastRecord') {
			$refundDetailObject->lastRecord('json');
		}
	}
	/*
	 * Excel Reporting
