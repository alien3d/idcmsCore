...
class InvoiceClass extends ConfigClass {
	...
	public function create() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			$sql="
			INSERT INTO `".$this->q->getFinancialDatabase()."`.`invoice` 
				(    
						`invoiceId`,   `invoiceTypeId`,   `documentNo`,`referenceNo`,   `invoiceTitle`,    `invoiceDesc`, `invoiceDate`, `invoiceStartDate`,    `invoiceEndDate`,  `invoiceAmount`,   `isDefault`,    `isNew`,    `isDraft`,  `isUpdate`, `isDelete`, `isActive`, `isApproved`,   `isReview`, `isPost`,   `executeBy`,    `executeTime`) VALUES ( null, '".$this->model->getInvoiceTypeId()."',
'".$this->model->getDocumentNo()."',
'".$this->model->getReferenceNo()."',
'".$this->model->getInvoiceTitle()."',
'".$this->model->getInvoiceDesc()."',
'".$this->model->getInvoiceDate()."',
'".$this->model->getInvoiceStartDate()."',
'".$this->model->getInvoiceEndDate()."',
'".$this->model->getInvoiceAmount()."',
...
".$this->model->getExecuteTime().");";
	...
	public function read() {
		...
		if ($this->isAdmin == 0) {
			if ($this->q->vendor == self::MYSQL) {
				$this->auditFilter = "		`invoice`.`isActive`		=	1
										AND	`invoice`.`isPost`		=	0";
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`invoice`.`invoiceId`,
					...
					`invoiceType`.`invoiceTypeDesc`,
                   	`staff`.`staffName`
            FROM    `".$this->q->getFinancialDatabase()."`.`invoice`
			...
			if ($this->model->getInvoiceId(0, 'single')) {
				$sql .= " AND `" . $this->model->getTableName() . "`.`" . $this->model->getPrimaryKeyName() . "`='" . $this->model->getInvoiceId(0, 'single') . "'";
			}


		} else if ($this->getVendor() == self::MSSQL) {
			$sql = "
			SELECT		[".$this->q->getFinancialDatabase()."].[invoice].[invoiceId],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceTypeId],
						[".$this->q->getFinancialDatabase()."].[invoice].[documentNo],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceTitle],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceDesc],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceDate],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceStartDate],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceEndDate],
						[".$this->q->getFinancialDatabase()."].[invoice].[invoiceAmount],
						[".$this->q->getFinancialDatabase()."].[invoice].[isDefault],
						[".$this->q->getFinancialDatabase()."].[invoice].[isNew],
						[".$this->q->getFinancialDatabase()."].[invoice].[isDraft],
						[".$this->q->getFinancialDatabase()."].[invoice].[isUpdate],
						[".$this->q->getFinancialDatabase()."].[invoice].[isDelete],
						[".$this->q->getFinancialDatabase()."].[invoice].[isActive],
						[".$this->q->getFinancialDatabase()."].[invoice].[isApproved],
						[".$this->q->getFinancialDatabase()."].[invoice].[isReview],
						[".$this->q->getFinancialDatabase()."].[invoice].[isPost],
						[".$this->q->getFinancialDatabase()."].[invoice].[executeBy],
						[".$this->q->getFinancialDatabase()."].[invoice].[executeTime],
						[iManagement].[staff].[staffName]
			FROM 	[".$this->q->getFinancialDatabase()."].[invoice]
			JOIN	[iManagement].[staff]
			ON		[".$this->q->getFinancialDatabase()."].[invoice].[executeBy] = [iManagement].[staff].[staffId]
			WHERE 	" . $this->auditFilter;
			//	if ($this->model->getInvoiceId(0, 'single')) {
			//	$sql .= " AND [" . $this->model->getTableName() . "].[" . $this->model->getPrimaryKeyName() . "]='" . $this->model->getInvoiceId(0, 'single') . "'";
			//}
			...
			SELECT		INVOICE.INVOICEID   		 	AS 	\"invoiceId\",
						INVOICE.DOCUMENTNO 				AS 	\"documentNo\",
						INVOICE.INVOICETITLE 			AS 	\"invoiceTitle\",
						INVOICE.INVOICEDESC 			AS 	\"invoiceDesc\",
			...
			WHERE 	" . $this->auditFilter;
			//	if ($this->model->getInvoiceId(0, 'single')) {
			//		$sql .= " AND " . strtoupper($this->model->getTableName()) . "." . strtoupper($this->model->getPrimaryKeyName()) . "='" . $this->model->getInvoiceId(0, 'single') . "'";
			//	}
		} else if ($this->q->vendor == self::DB2) {
		 ...
		 */
		$filterArray = null;
		$filterArray = array('invoiceId');
		/**
		 * filter table
		 * @variables $tableArray
		 */
		$tableArray = null;
		$tableArray = array('invoice');
				...
				$sql = "
							...
							(
								SELECT 		[".$this->q->getFinancialDatabase()."].[invoice].[invoiceId],
											...
											[".$this->q->getFinancialDatabase()."].[invoice].[executeTime],
											[iManagement].[staff].[staffName],
								ROW_NUMBER() OVER (ORDER BY [".$this->q->getFinancialDatabase()."].[invoice].[invoiceId]) AS 'RowNumber'
								FROM 	[".$this->q->getFinancialDatabase()."].[invoice]
								JOIN		[iManagement].[staff]
								ON		[".$this->q->getFinancialDatabase()."].[invoice].[executeBy] = [iManagement].[staff].[staffId]
	...
	function update() {
		...
		if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	[" . $this->model->getTableName() . "]	[" . $this->model->getPrimaryKeyName() . "]
			FROM 	[".$this->q->getFinancialDatabase()."].[" . $this->model->getTableName() . "]
			WHERE  	[".$this->q->getFinancialDatabase()."].[" . $this->model->getTableName() . "][" . $this->model->getPrimaryKeyName() . "] = '" . $this->model->getInvoiceId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::ORACLE) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
			WHERE  	" . strtoupper($this->model->getPrimaryKeyName()) . " = '" . $this->model->getInvoiceId(0, 'single') . "' ";
		} else if ($this->getVendor() == self::DB2) {
			$sql = "
			SELECT	" . strtoupper($this->model->getPrimaryKeyName()) . "
			FROM 	" . strtoupper($this->model->getTableName()) . "
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE		`".$this->q->getFinancialDatabase()."`.`invoice`
				SET 		`invoiceTypeId` = '".$this->model->getInvoiceTypeId()."',
							 `invoiceTitle` = '".$this->model->getInvoiceTitle()."',
							 `invoiceDesc` = '".$this->model->getInvoiceDesc()."',
							 `invoiceDate` = '".$this->model->getInvoiceDate()."',
							 `invoiceStartDate` = '".$this->model->getInvoiceStartDate()."',
							 `invoiceEndDate` = '".$this->model->getInvoiceEndDate()."',
							 `invoiceAmount` = '".$this->model->getInvoiceAmount()."',
	...
	function delete() {
		...
		if ($total == 0) {
			...
			if ($this->getVendor() == self::MYSQL) {
				$sql = "
				UPDATE 	`".$this->q->getFinancialDatabase()."`.`invoice`
				SET 	`invoice`.`isDefault`			=	'" . $this->model->getIsDefault(0, 'single') . "',
						`invoice`.`isNew`				=	'" . $this->model->getIsNew(0, 'single') . "',
						`invoice`.`isDraft`			=	'" . $this->model->getIsDraft(0, 'single') . "',
						`invoice`.`isUpdate`			=	'" . $this->model->getIsUpdate(0, 'single') . "',
						`invoice`.`isDelete`			=	'" . $this->model->getIsDelete(0, 'single') . "',
						`invoice`.`isActive`			=	'" . $this->model->getIsActive(0, 'single') . "',
						`invoice`.`isApproved`		=	'" . $this->model->getIsApproved(0, 'single') . "',
						`invoice`.`isReview`			=	'" . $this->model->getIsReview(0, 'single') . "',
						`invoice`.`isPost`			=	'" . $this->model->getIsPost(0, 'single') . "',
						`invoice`.`executeBy`			=	'" . $this->model->getExecuteBy() . "',
						`invoice`.`executeTime`		=	" . $this->model->getExecuteTime() . "
				WHERE 	`invoiceId`		=	'" . $this->model->getInvoiceId(0, 'single') . "'";
			} else if ($this->getVendor() == self::MSSQL) {
				$sql = "
				UPDATE 	[".$this->q->getFinancialDatabase()."].[invoice]
				...
				case 'isDefault' :
					for ($i = 0; $i < $loop; $i++) {
						if (strlen($this->model->getIsDefault($i, 'array')) > 0) {
							...
							$sqlLooping .= "
							WHEN '" . $this->model->getInvoiceId($i, 'array') . "'
	...
	function closingChecking(){
		$arrayDate = explode("-",$this->model->getInvoiceDate());
	...
	function duplicate() {
		...
		if ($total > 0) {
			...
			echo json_encode(array("success" => true, "total" => $total, "message" => $this->systemString->getDuplicateMessage(), "invoiceDesc" => $row ['invoiceDesc']));
			exit();
		} else {
	...
	function masterDetailChecking(){
		$sqlMaster="
			...
			SELECT `invoiceAmount` as total
			FROM 	`".$this->q->getFinancialDatabase()."`.`invoice`
			WHERE   `invoice`.`invoiceId`='".$this->model->getInvoiceId(0,'single')."'
		";
		...
		$sqlDetail="	
			SELECT `invoiceDetailAmount` as total
			FROM 	`".$this->q->getFinancialDatabase()."`.`invoiceDetail`
			WHERE   `invoiceDetail`.`invoiceId`='".$this->model->getInvoiceId(0,'single')."'
	...
	function trialBalanceChecking(){
		...
		$sqlAsset="
		SELECT 	SUM(`invoiceDetailAmount`) as `total`
		FROM 	`".$this->q->getFinancialDatabase()."`.`invoiceDetail`
	...
	function tallyChecking(){
		...
		USING	(`generalLedgerChartOfAccountId`)
		WHERE	`generalLedgerChartOfAccount`.`isActive`=1
		AND		`invoiceDetail`.`transactionMode`='D'
		AND		`invoiceDetail`.`isActive`=1 
		WHERE   `invoiceDetail`.`invoiceId`='".$this->model->getInvoiceId(0,'single')."'";
		...
		USING	(`generalLedgerChartOfAccountId`)
		WHERE	`generalLedgerChartOfAccount`.`isActive`=1
		AND		`invoiceDetail`.`transactionMode`='C'
		AND		`invoiceDetail`.`isActive`=1 
		WHERE   `invoiceDetail`.`invoiceId`='".$this->model->getInvoiceId(0,'single')."'";
	...
	function posting() {
		...
		SELECT 	`invoiceDetail`.`invoiceDetailId`, 
			   	...
			   	`invoiceDetail`.`generalLedgerChartOfAccountId`, 
			  	`invoiceDetail`.`countryId`, 
			  	`invoiceDetail`.`transactionMode`, 
			  	`invoiceDetail`.`invoiceDetailAmount` AS `generalLedgerLocalAmount`,  
			    ...
			    `invoice`.`documentNo`,
			    `invoice`.`invoiceTitle` AS `generalLedgerTitle`,  
			    `invoice`.`invoiceDesc` AS `generalLedgerDesc`,
				`invoice`.`invoiceDate` AS `generalLedgerDate`,
		...
		WHERE  `invoiceDetail`.`invoiceId`='".$this->model->getInvoiceId(0,'single')."'";
		//echo "<br>--------".$sqlInvoiceDetail."-------";
		$resultInvoiceDetail=$this->q->fast($sqlInvoiceDetail);
		while($row = $this->q->fetchArray($resultInvoiceDetail)) {
			$row['isDefault']=0;
			$row['isNew']=1;
			$row['isDraft']=0;
			$row['isUpdate']=0;
		 ...
		 */
		$sqlInvoice="
		UPDATE 	`".$this->q->getFinancialDatabase()."`.`invoiceDetail` 
		SET 	`isPost`=1 
		WHERE 	`invoiceId`='".$this->model->getInvoiceId(0, 'single')."'";
		$this->q->update($sqlInvoice);
		if ($this->q->execute == 'fail') {
	...
	function excel() {
		...
		while (($row = $this->q->fetchAssoc()) == TRUE) {
			...
			$this->excel->getActiveSheet()->setCellValue('C' . $loopRow, 'a' . $row ['invoiceDesc']);
			$loopRow++;
			$lastRow = 'C' . $loopRow;
		}
		$from = 'B2';
		...
		$filename = "invoice" . rand(0, 10000000) . ".xlsx";
...
if (isset($_POST ['method'])) {
	...
	if (isset($_POST ['isAdmin'])) {
		$invoiceObject->setIsAdmin($_POST ['isAdmin']);
	}
	/*
	 *  Paging
	 */
	if (isset($_POST ['start'])) {
		$invoiceObject->setStart($_POST ['start']);
	}
	if (isset($_POST ['perPage'])) {
		$invoiceObject->setLimit($_POST ['perPage']);
	}
	/*
	 *  Filtering
	 */
	if (isset($_POST ['query'])) {
		$invoiceObject->setFieldQuery($_POST ['query']);
	}
	if (isset($_POST ['filter'])) {
		$invoiceObject->setGridQuery($_POST ['filter']);
	}
	if (isset($_POST ['character'])) {
		$invoiceObject->setCharacterQuery($_POST['character']);
	}
	if (isset($_POST ['dateRangeStart'])) {
		$invoiceObject->setDateRangeStartQuery($_POST['dateRangeStart']);
	}
	if (isset($_POST ['dateRangeEnd'])) {
		$invoiceObject->setDateRangeEndQuery($_POST['dateRangeEnd']);
	}
	if (isset($_POST ['dateRangeType'])) {

		$invoiceObject->setDateRangeTypeQuery($_POST['dateRangeType']);
	}
	/*
	 * Ordering
	 */
	if (isset($_POST ['order'])) {
		$invoiceObject->setOrder($_POST ['order']);
	}
	if (isset($_POST ['sortField'])) {
		$invoiceObject->setSortField($_POST ['sortField']);
	}
	/*
...
if (isset($_GET ['method'])) {
	...
	if (isset($_GET ['isAdmin'])) {
		$invoiceObject->setIsAdmin($_GET ['isAdmin']);
	}
	/*
	 *  Load the dynamic value
	 */
	 ...
	 */
	if (isset($_GET ['invoiceDesc'])) {
		if (strlen($_GET ['invoiceDesc']) > 0) {
