Ext.onReady(function () {
	...
	// start invoiceAllocation request
	var invoiceAllocationProxy = new Ext.data.HttpProxy({
			url : '../controller/invoiceAllocationController.php',
			method : 'POST',
			success : function (response, options) {
				jsonResponse = Ext.decode(response.responseText);
				if (jsonResponse.success == true) {
	...
	var invoiceAllocationReader = new Ext.data.JsonReader({
			totalProperty : 'total',
			successProperty : 'success',
			messageProperty : 'message',
			idProperty : 'invoiceAllocationId'
		});
	
	var invoiceAllocationStore = new Ext.data.JsonStore({
			proxy : invoiceAllocationProxy,
			reader : invoiceAllocationReader,
			autoLoad : true,
			autoDestroy : true,
			pruneModifiedRecords : true,
			baseParams : {
	...
	// end vourcher detail
	// start invoice allocation
	var invoiceAllocationFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : [{
					type : 'int',
					dataIndex : 'invoiceAllocationId',
					column : 'invoiceAllocationId',
					...
					type : 'float',
					dataIndex : 'invoiceAllocationAmount',
					column : 'invoiceAllocationAmount',
					table : 'invoiceAllocation',
					database : 'ifinancial'
				}, {
					type : 'boolean',
	...
	var invoiceAllocationId = new Ext.form.Hidden({
			name : 'invoiceAllocationId',
			id : 'invoiceAllocationId'
		});
	...
	var invoiceLedgerId = new Ext.ux.form.ComboBoxMatch({
			...
			labelAlign : 'left',
			fieldLabel : invoiceLedgerIdLabel,
			name : 'stateId',
	...
	var invoiceAllocationAmount = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : invoiceAllocationAmountLabel + '<span style="\'color:" red;\'="">*</span>',
			hiddenName : 'invoiceAllocationAmount',
			name : 'invoiceAllocationAmount',
			id : 'invoiceAllocationAmount',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
			...
			listeners : {
				blur : function () {
					var value = Ext.getCmp('invoiceAllocationAmount').getValue();
					value = value.replace(",", "");
					value = value.replace(" ", "");
					Ext.getCmp('invoiceAllocationAmount').setValue(value);
				}
			}
		});
	...
	var invoiceAllocationColumnModel = [
		new Ext.grid.RowNumberer(), {
			dataIndex : 'invoiceLedgerId',
			header : invoiceLedgerDescLabel,
			width : 200,
			sortable : true,
			editor : invoiceLedgerId,
			renderer : Ext.util.Format.comboRenderer(invoiceLedgerId),
			hidden : false,
			jsonType : 'int'
		}, {
			dataIndex : 'invoiceLedgerDate',
			...
			dataIndex : 'invoiceAllocationAmount',
			header : invoiceAllocationAmountLabel,
			width : 75,
			sortable : true,
			summaryType : 'sum',
			renderer : function (value) {
			...
			editor : {
				xtype : 'textfield',
				labelAlign : 'left',
				fieldLabel : invoiceAllocationAmountLabel,
				hiddenName : 'invoiceAllocationAmount',
				name : 'invoiceAllocationAmount',
				id : 'invoiceAllocationAmount',
	...
	var invoiceAllocationEditor = new Ext.ux.grid.RowEditor({
			saveText : saveButtonLabel,
			cancelText : cancelButtonLabel,
			listeners : {
				cancelEdit : function (rowEditor, changes, record, rowIndex) {
					
					invoiceAllocationStore.reload({
						params : {
							voucherLedgerId : Ext.getCmp('voucherLedgerId').getValue()
						}
					});
				...
				afteredit : function (rowEditor, changes, record, rowIndex) {
					this.save = true;
					var record = this.grid.getStore().getAt(rowIndex);
					if (parseInt(record.get('invoiceAllocationId')) == 'NaN') {
						method = 'create';
					} else if (record.get('invoiceAllocationId') == '') {
						method = 'create';
					} else if (record.get('invoiceAllocationId') == undefined) {
						method = 'create';
					} else if (parseInt(record.get('invoiceAllocationId')) > 0) {
						method = 'save';
					} else {
						method = 'create';
					}
					Ext.Ajax.request({
						...
						params : {
							...
							isAdmin : isAdmin,
							method : method,
							invoiceAllocationId : record.get('invoiceAllocationId'),
							invoiceLedgerId : record.get('invoiceLedgerId'),
							voucherLedgerId : Ext.getCmp('voucherLedgerId').getValue(),
							invoiceAllocationAmount : record.get('invoiceAllocationAmount')
						},
						success : function (response, options) {
							jsonResponse = Ext.decode(response.responseText);
							if (jsonResponse.success == false) {
	...
	var invoiceAllocationEntity = Ext.data.Record.create([{
					key : 'PRI',
					foreignKey : 'no',
					name : 'invoiceAllocationId',
					type : 'int'
	...
	var invoiceAllocationFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var invoiceAllocationGrid = new Ext.grid.GridPanel({
			title:'Knock Off Invoice /Debit Note',
			name : 'invoiceAllocationGrid',
			id : 'invoiceAllocationGrid',
			border : false,
			store : invoiceAllocationStore,
			height : 250,
			autoScroll : true,
			columns : invoiceAllocationColumnModel,
			viewConfig : {
				autoFill : true,
				forceFit : true,
				emptyRow : emptyRowLabel
			...
			layout : 'fit',
			disabled : true,
			plugins : [invoiceAllocationEditor],
			tbar : {
				items : [{
						xtype : 'button',
						iconCls : 'add',
						...
						text : newButtonLabel,
						handler : function () {
							var newRecord = new invoiceAllocationEntity({
									invoiceAllocationId : '',
									invoiceLedgerId : '',
									voucherLedgerId : '',
							...
							invoiceAllocationEditor.stopEditing();
							invoiceAllocationStore.insert(0, newRecord);
							invoiceAllocationGrid.getSelectionModel().getSelections();
							invoiceAllocationEditor.startEditing(0);
						}
					}, {
						xtype : 'button',
						text : CheckAllLabel,
						...
						listeners : {
							'click' : function (button, e) {
								invoiceAllocationStore.each(function (record, fn, scope) {
									for (var access in invoiceAllocationFlagArray) {
										record.set(invoiceAllocationFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								invoiceAllocationStore.each(function (record, fn, scope) {
									for (var access in invoiceAllocationFlagArray) {
										record.set(invoiceAllocationFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								var url = '../controller/invoiceAllocationController.php?';
								var sub_url = '';
								var modified = invoiceAllocationStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&invoiceAllocationId[]=' + modified[i].get('invoiceAllocationId');
									if (isAdmin == 1) {
										if (dataChanges.isDefault == true || dataChanges.isDefault == false) {
											sub_url = sub_url + '&isDefault[]=' + modified[i].get('isDefault');
										}
