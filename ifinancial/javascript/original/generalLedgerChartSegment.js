Ext.onReady(function () {
	...
	var staffByProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerSegmentController.php?',
			method : 'GET',
	...
	var generalLedgerSegmentProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerSegmentController.php',
			method : 'POST',
			success : function (response, options) {
				jsonResponse = Ext.decode(response.responseText);
	...
	var generalLedgerSegmentReader = new Ext.data.JsonReader({
			totalProperty : 'total',
			successProperty : 'success',
			messageProperty : 'message',
			idProperty : 'generalLedgerSegmentId'
		});
	var generalLedgerSegmentStore = new Ext.data.JsonStore({
			proxy : generalLedgerSegmentProxy,
			reader : generalLedgerSegmentReader,
			autoLoad : true,
			autoDestroy : true,
			pruneModifiedRecords : true,
			baseParams : {
			...
			root : 'data',
			fields : [{
					name : 'generalLedgerSegmentId',
					type : 'int'
				}, {
					name : 'generalLedgerSegmentNo',
					type : 'string'
				}, {
					name : 'generalLedgerSegmentLength',
					type : 'string'
				}, {
					name : 'generalLedgerSegmentTitle',
					type : 'string'
				}, {
					name : 'generalLedgerSegmentDesc',
					type : 'string'
				}, {
					name : 'isDefault',
					type : 'boolean'
	...
	var generalLedgerSegmentFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : {
					type : 'string',
					dataIndex : 'generalLedgerSegmentNo',
					column : 'generalLedgerSegmentNo',
					table : 'generalLedgerSegment',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'generalLedgerSegmentLength',
					column : 'generalLedgerSegmentLength',
					table : 'generalLedgerSegment',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'generalLedgerSegmentTitle',
					column : 'generalLedgerSegmentTitle',
					table : 'generalLedgerSegment',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'generalLedgerSegmentDesc',
					column : 'generalLedgerSegmentDesc',
					table : 'generalLedgerSegment',
					database : 'iFinancial'
				}, {
					type : 'list',
	...
	var generalLedgerSegmentColumnModel = [new Ext.grid.RowNumberer(), {
	...
	var generalLedgerSegmentFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var generalLedgerSegmentGrid = new Ext.grid.GridPanel({
			name : 'generalLedgerSegmentGrid',
			id : 'generalLedgerSegmentGrid',
			border : false,
			store : generalLedgerSegmentStore,
			autoHeight : false,
			height : 400,
			columns : generalLedgerSegmentColumnModel,
			plugins : [generalLedgerSegmentFilters],
			selModel : new Ext.grid.RowSelectionModel({
				singleSelect : true
			}),
			viewConfig : {
			...
			listeners : {
				'rowclick' : function (object, rowIndex, e) {
					var record = generalLedgerSegmentStore.getAt(rowIndex);
					...
					formPanel.form.load({
						...
						params : {
							method : 'read',
							mode : 'update',
							generalLedgerSegmentId : record.data.generalLedgerSegmentId,
							leafId : leafId,
							isAdmin : isAdmin
						},
						success : function (form, action) {
							Ext.getCmp('generalLedgerSegmentCodeTemp').setValue(record.data.generalLedgerSegmentDesc);
				...
				items : [{
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerSegmentStore.each(function (record, fn, scope) {
									for (var access in generalLedgerSegmentFlagArray) {
										record.set(generalLedgerSegmentFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerSegmentStore.each(function (record, fn, scope) {
									for (var access in generalLedgerSegmentFlagArray) {
										record.set(generalLedgerSegmentFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								var url = '../controller/generalLedgerSegmentController.php?';
								var sub_url = '';
								var modified = generalLedgerSegmentStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&generalLedgerSegmentId[]=' + modified[i].get('generalLedgerSegmentId');
									if (isAdmin == 1) {
								...
								Ext.Ajax.request({
									...
									success : function (response, options) {
										...
										if (jsonResponse.success == true) {
											...
											generalLedgerSegmentStore.reload();
	...
	var gridPanel = new Ext.Panel({
			...
			items : [generalLedgerSegmentGrid]
		});
	// form entry
	...
	var generalLedgerSegmentId = new Ext.form.Hidden({
			name : 'generalLedgerSegmentId',
			id : 'generalLedgerSegmentId'
		}); // end form entry
	...
	var formPanel = new Ext.form.FormPanel({
			...
			items : [{
					xtype : 'fieldset',
					title : 'Form Entry',
					items : [generalLedgerSegmentId,
						generalLedgerSegmentNo,
						generalLedgerSegmentLength,
						generalLedgerSegmentTitle,
						generalLedgerSegmentDesc]
				}, {
					xtype : 'fieldset',
			...
			buttons : [{
					...
					iconCls : 'new',
					handler : function () {
						var id = Ext.getCmp('generalLedgerSegmentId').getValue();
						var method = 'create';
						formPanel.getForm().submit({
							...
							success : function (form, action) {
								if (action.result.success == true) {
									...
									Ext.getCmp('generalLedgerSegmentId').setValue(action.result.generalLedgerSegmentId);
								} else {
									Ext.MessageBox.alert(systemErrorLabel, action.result.message);
								}
							},
							...
							fn : function (response) {
								if ('yes' == response) {
									Ext.Ajax.request({
										...
										params : {
											method : 'delete',
											generalLedgerSegmentId : Ext.getCmp('generalLedgerSegmentId').getValue(),
											leafId : leafId,
											isAdmin : isAdmin
										},
										success : function (response, options) {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerSegmentId : Ext.getCmp('firstRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('firstRecord').getValue() >= 1) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerSegmentId : Ext.getCmp('previousRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerSegmentId : Ext.getCmp('nextRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerSegmentId : Ext.getCmp('lastRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
						...
						if (Ext.getCmp('generalLedgerSegmentId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								url : '../controller/generalLedgerSegmentController.php',
								method : 'POST',
								waitTitle : systemLabel,
