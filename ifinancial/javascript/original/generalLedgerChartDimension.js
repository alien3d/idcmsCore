Ext.onReady(function () {
	...
	var staffByProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerDimensionController.php?',
			method : 'GET',
	...
	var generalLedgerDimensionProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerDimensionController.php',
			method : 'POST',
			success : function (response, options) {
				jsonResponse = Ext.decode(response.responseText);
	...
	var generalLedgerDimensionReader = new Ext.data.JsonReader({
			totalProperty : 'total',
			successProperty : 'success',
			messageProperty : 'message',
			idProperty : 'generalLedgerDimensionId'
		});
	var generalLedgerDimensionStore = new Ext.data.JsonStore({
			proxy : generalLedgerDimensionProxy,
			reader : generalLedgerDimensionReader,
			autoLoad : true,
			autoDestroy : true,
			pruneModifiedRecords : true,
			baseParams : {
			...
			root : 'data',
			fields : [{
					name : 'generalLedgerDimensionId',
					type : 'int'
				}, {
					name : 'generalLedgerDimensionTitle',
					type : 'string'
				}, {
					name : 'generalLedgerDimensionDesc',
					type : 'string'
				}, {
	...
	var generalLedgerDimensionFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : [{
					type : 'string',
					dataIndex : 'generalLedgerDimensionTitle',
					column : 'generalLedgerDimensionTitle',
					table : 'generalLedgerDimension',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'generalLedgerDimensionDesc',
					column : 'generalLedgerDimensionDesc',
					table : 'generalLedgerDimension',
					database : 'iFinancial'
				},{
					type : 'list',
	...
	var generalLedgerDimensionColumnModel = [new Ext.grid.RowNumberer(), {
			...
			dataIndex : 'generalLedgerDimensionTitle',
			header : generalLedgerDimensionTitleLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
			dataIndex : 'generalLedgerDimensionDesc',
			header : generalLedgerDimensionDescLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
	...
	var generalLedgerDimensionFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var generalLedgerDimensionGrid = new Ext.grid.GridPanel({
			name : 'generalLedgerDimensionGrid',
			id : 'generalLedgerDimensionGrid',
			border : false,
			store : generalLedgerDimensionStore,
			autoHeight : false,
			height : 400,
			columns : generalLedgerDimensionColumnModel,
			plugins : [generalLedgerDimensionFilters],
			selModel : new Ext.grid.RowSelectionModel({
				singleSelect : true
			}),
			viewConfig : {
			...
			listeners : {
				'rowclick' : function (object, rowIndex, e) {
					var record = generalLedgerDimensionStore.getAt(rowIndex);
					...
					formPanel.form.load({
						...
						params : {
							method : 'read',
							mode : 'update',
							generalLedgerDimensionId : record.data.generalLedgerDimensionId,
							leafId : leafId,
							isAdmin : isAdmin
						},
						success : function (form, action) {
							Ext.getCmp('generalLedgerDimensionCodeTemp').setValue(record.data.generalLedgerDimensionDesc);
				...
				items : [{
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerDimensionStore.each(function (record, fn, scope) {
									for (var access in generalLedgerDimensionFlagArray) {
										record.set(generalLedgerDimensionFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerDimensionStore.each(function (record, fn, scope) {
									for (var access in generalLedgerDimensionFlagArray) {
										record.set(generalLedgerDimensionFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								var url = '../controller/generalLedgerDimensionController.php?';
								var sub_url = '';
								var modified = generalLedgerDimensionStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&generalLedgerDimensionId[]=' + modified[i].get('generalLedgerDimensionId');
									if (isAdmin == 1) {
								...
								Ext.Ajax.request({
									...
									success : function (response, options) {
										...
										if (jsonResponse.success == true) {
											...
											generalLedgerDimensionStore.reload();
	...
	var gridPanel = new Ext.Panel({
			...
			items : [generalLedgerDimensionGrid]
		});
	// form entry
	var generalLedgerDimensionTitle = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : generalLedgerDimensionTitleLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerDDesc',
			name : 'generalLedgerDimensionDesc',
			id : 'generalLedgerDimensionDesc',
			allowBlank : false,
	...
	var generalLedgerDimensionDesc = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : generalLedgerDimensionDescLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerDimensionDesc',
			name : 'generalLedgerDimensionDesc',
			id : 'generalLedgerDimensionDesc',
			allowBlank : false,
			blankText : blankTextLabel,
	...
	var generalLedgerDimensionId = new Ext.form.Hidden({
			name : 'generalLedgerDimensionId',
			id : 'generalLedgerDimensionId'
		}); // end form entry
	...
	var formPanel = new Ext.form.FormPanel({
			...
			items : [{
					xtype : 'fieldset',
					title : 'Form Entry',
					items : [generalLedgerDimensionId,
						
						generalLedgerDimensionTitle,
						generalLedgerDimensionDesc,
						generalLedgerChartAccountIdStart,
						generalLedgerChartAccountIdEnd]
				}, {
					xtype : 'fieldset',
			...
			buttons : [{
					...
					iconCls : 'new',
					handler : function () {
						var id = Ext.getCmp('generalLedgerDimensionId').getValue();
						var method = 'create';
						formPanel.getForm().submit({
							...
							success : function (form, action) {
								if (action.result.success == true) {
									...
									Ext.getCmp('generalLedgerDimensionId').setValue(action.result.generalLedgerDimensionId);
								} else {
									Ext.MessageBox.alert(systemErrorLabel, action.result.message);
								}
							},
							...
							fn : function (response) {
								if ('yes' == response) {
									Ext.Ajax.request({
										...
										params : {
											method : 'delete',
											generalLedgerDimensionId : Ext.getCmp('generalLedgerDimensionId').getValue(),
											leafId : leafId,
											isAdmin : isAdmin
										},
										success : function (response, options) {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerDimensionId : Ext.getCmp('firstRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('firstRecord').getValue() >= 1) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerDimensionId : Ext.getCmp('previousRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerDimensionId : Ext.getCmp('nextRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerDimensionId : Ext.getCmp('lastRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
						...
						if (Ext.getCmp('generalLedgerDimensionId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								url : '../controller/generalLedgerDimensionController.php',
								method : 'POST',
								waitTitle : systemLabel,
