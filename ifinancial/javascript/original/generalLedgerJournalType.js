Ext.onReady(function () {
	...
	var staffByProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerJournalTypeController.php?',
			method : 'GET',
	...
	var generalLedgerJournalTypeStore = new Ext.data.JsonStore({
			...
			fields : [{
					...
					name : 'generalLedgerJournalTypeCode',
					type : 'string'
				}, {
	...
	var generalLedgerJournalTypeFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : [ {
					type : 'string',
					dataIndex : 'generalLedgerJournalTypeSequence',
					column : 'generalLedgerJournalTypeSequence',
					table : 'generalLedgerJournalType',
					database : 'iFinancial'
				},{
					type : 'string',
					dataIndex : 'generalLedgerJournalTypeCode',
					column : 'generalLedgerJournalTypeCode',
					table : 'generalLedgerJournalType',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'generalLedgerJournalTypeDesc',
					column : 'generalLedgerJournalTypeDesc',
					table : 'generalLedgerJournalType',
					database : 'iFinancial'
				}, {
					type : 'list',
	...
	var generalLedgerJournalTypeColumnModel = [new Ext.grid.RowNumberer(),{
			dataIndex : 'generalLedgerJournalTypeCode',
			header : generalLedgerJournalTypeCodeLabel,
			sortable : true,
			hidden : false,
			width : 50
		},{
			dataIndex : 'generalLedgerJournalTypeSequence',
			header : generalLedgerJournalTypeSequenceLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
			dataIndex : 'generalLedgerJournalTypeDesc',
			header : generalLedgerJournalTypeDescLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
	...
	var generalLedgerJournalTypeFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var generalLedgerJournalTypeGrid = new Ext.grid.GridPanel({
			name : 'generalLedgerJournalTypeGrid',
			id : 'generalLedgerJournalTypeGrid',
			border : false,
			store : generalLedgerJournalTypeStore,
			autoHeight : false,
			height : 400,
			columns : generalLedgerJournalTypeColumnModel,
			plugins : [generalLedgerJournalTypeFilters],
			selModel : new Ext.grid.RowSelectionModel({
				singleSelect : true
			}),
			viewConfig : {
			...
			listeners : {
				'rowclick' : function (object, rowIndex, e) {
					var record = generalLedgerJournalTypeStore.getAt(rowIndex);
					...
					formPanel.form.load({
						...
						params : {
							method : 'read',
							mode : 'update',
							generalLedgerJournalTypeId : record.data.generalLedgerJournalTypeId,
							leafId : leafId,
							isAdmin : isAdmin
						},
						success : function (form, action) {
							Ext.getCmp('generalLedgerJournalTypeCodeTemp').setValue(record.data.generalLedgerJournalTypeDesc);
				...
				items : [{
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerJournalTypeStore.each(function (record, fn, scope) {
									for (var access in generalLedgerJournalTypeFlagArray) {
										record.set(generalLedgerJournalTypeFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerJournalTypeStore.each(function (record, fn, scope) {
									for (var access in generalLedgerJournalTypeFlagArray) {
										record.set(generalLedgerJournalTypeFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								var url = '../controller/generalLedgerJournalTypeController.php?';
								var sub_url = '';
								var modified = generalLedgerJournalTypeStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&generalLedgerJournalTypeId[]=' + modified[i].get('generalLedgerJournalTypeId');
									if (isAdmin == 1) {
								...
								Ext.Ajax.request({
									...
									success : function (response, options) {
										...
										if (jsonResponse.success == true) {
											...
											generalLedgerJournalTypeStore.reload();
	...
	var gridPanel = new Ext.Panel({
			...
			items : [generalLedgerJournalTypeGrid]
		});
	// form entry
	
	var generalLedgerJournalTypeCode = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : generalLedgerJournalTypeCodeLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerJournalTypeCode',
			name : 'generalLedgerJournalTypeCode',
			id : 'generalLedgerJournalTypeCode',
			allowBlank : false,
	...
	var generalLedgerJournalTypeSequence = new Ext.form.NumberField({
			labelAlign : 'left',
			fieldLabel : generalLedgerJournalTypeSequenceLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerJournalTypeSequence',
			name : 'generalLedgerJournalTypeSequence',
			id : 'generalLedgerJournalTypeSequence',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var generalLedgerJournalTypeDesc = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : generalLedgerJournalTypeDescLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerJournalTypeDesc',
			name : 'generalLedgerJournalTypeDesc',
			id : 'generalLedgerJournalTypeDesc',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var generalLedgerJournalTypeId = new Ext.form.Hidden({
			name : 'generalLedgerJournalTypeId',
	...
	var formPanel = new Ext.form.FormPanel({
			...
			items : [{
					xtype : 'fieldset',
					title : 'Form Entry',
					items : [generalLedgerJournalTypeId,
			...
			buttons : [{
					...
					iconCls : 'new',
					handler : function () {
						var id = Ext.getCmp('generalLedgerJournalTypeId').getValue();
						var method = 'create';
						formPanel.getForm().submit({
							...
							success : function (form, action) {
								if (action.result.success == true) {
									...
									Ext.getCmp('generalLedgerJournalTypeId').setValue(action.result.generalLedgerJournalTypeId);
								} else {
									Ext.MessageBox.alert(systemErrorLabel, action.result.message);
								}
							},
							...
							fn : function (response) {
								if ('yes' == response) {
									Ext.Ajax.request({
										...
										params : {
											method : 'delete',
											generalLedgerJournalTypeId : Ext.getCmp('generalLedgerJournalTypeId').getValue(),
											leafId : leafId,
											isAdmin : isAdmin
										},
										success : function (response, options) {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerJournalTypeId : Ext.getCmp('firstRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('firstRecord').getValue() >= 1) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerJournalTypeId : Ext.getCmp('previousRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerJournalTypeId : Ext.getCmp('nextRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerJournalTypeId : Ext.getCmp('lastRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
						...
						if (Ext.getCmp('generalLedgerJournalTypeId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								url : '../controller/generalLedgerJournalTypeController.php',
								method : 'POST',
								waitTitle : systemLabel,
