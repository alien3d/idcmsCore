Ext.onReady(function () {
	...
	// start request General Ledger Chart Account
	var generalLedgerChartAccountProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerChartAccountController.php',
			method : 'POST',
			success : function (response, options) {
				jsonResponse = Ext.decode(response.responseText);
	...
	var generalLedgerChartAccountReader = new Ext.data.JsonReader({
			totalProperty : 'total',
			successProperty : 'success',
			messageProperty : 'message',
			idProperty : 'generalLedgerChartAccountId'
		});
	var generalLedgerChartAccountStore = new Ext.data.JsonStore({
			proxy : generalLedgerChartAccountProxy,
			reader : generalLedgerChartAccountReader,
			autoLoad : true,
			autoDestroy : true,
			pruneModifiedRecords : true,
			baseParams : {
			...
			fields : [{
					name : 'generalLedgerChartAccountId',
					type : 'int'
				}, {
					name : 'generalLedgerChartAccountNo',
					type : 'string'
				}, {
					name : 'generalLedgerChartAccountDesc',
					type : 'string'
				}
			]
		});
	// end request General Ledger Chart Account
	// end additional Proxy ,Reader,Store,Filter,Grid
	// start application Proxy ,Reader,Store,Filter,Grid
	var generalLedgerForecastProxy = new Ext.data.HttpProxy({
			url : '../controller/generalLedgerForecastController.php',
			method : 'POST',
			success : function (response, options) {
				jsonResponse = Ext.decode(response.responseText);
	...
	var generalLedgerForecastReader = new Ext.data.JsonReader({
			totalProperty : 'total',
			successProperty : 'success',
			messageProperty : 'message',
			idProperty : 'generalLedgerForecastId'
		});
	var generalLedgerForecastStore = new Ext.data.JsonStore({
			proxy : generalLedgerForecastProxy,
			reader : generalLedgerForecastReader,
			autoLoad : true,
			autoDestroy : true,
			pruneModifiedRecords : true,
			baseParams : {
			...
			fields : [{
					...
					name : 'generalLedgerForecastMonth',
					type : 'string'
				}, {
					name : 'generalLedgerForecastYear',
					type : 'string'
				}, {
					name : 'generalLedgerForecastAmount',
					type : 'string'
				}, {
					name : 'generalLedgerForecastDesc',
					type : 'string'
				}, {
					name : 'isDefault',
					type : 'boolean'
	...
	var generalLedgerForecastFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : [{
					type : 'string',
					...
					type : 'numeric',
					dataIndex : 'generalLedgerForecastMonth',
					column : 'generalLedgerForecastMonth',
					table : 'generalLedgerForecastMonth',
					database : 'iFinancial'
				}, {
					type : 'numeric',
					dataIndex : 'generalLedgerForecastYear',
					column : 'generalLedgerForecastYear',
					table : 'generalLedgerForecast',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'generalLedgerForecastDesc',
					column : 'generalLedgerForecastDesc',
					table : 'generalLedgerForecast',
					database : 'iFinancial'
				}, {
					type : 'list',
	...
	var generalLedgerForecastColumnModel = [new Ext.grid.RowNumberer(), {
			dataIndex : 'documentNo',
			...
			dataIndex : 'generalLedgerForecastMonth',
			header : generalLedgerForecastMonthLabel,
			sortable : true,
			hidden : false,
			width : 100
		}, {
			dataIndex : 'generalLedgerForecastAmount',
			header : generalLedgerForecastAmountLabel,
			sortable : true,
			hidden : false,
			width : 100
		}, {
			dataIndex : 'generalLedgerForecastDesc',
			header : generalLedgerForecastDescLabel,
			sortable : true,
			hidden : false,
			width : 50
		},
	...
	var generalLedgerForecastFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var generalLedgerForecastGrid = new Ext.grid.GridPanel({
			name : 'generalLedgerForecastGrid',
			id : 'generalLedgerForecastGrid',
			border : false,
			store : generalLedgerForecastStore,
			autoHeight : false,
			height : 400,
			columns : generalLedgerForecastColumnModel,
			plugins : [generalLedgerForecastFilters],
			selModel : new Ext.grid.RowSelectionModel({
				singleSelect : true
			}),
			viewConfig : {
			...
			listeners : {
				'rowclick' : function (object, rowIndex, e) {
					var record = generalLedgerForecastStore.getAt(rowIndex);
					...
					formPanel.form.load({
						...
						params : {
							method : 'read',
							mode : 'update',
							generalLedgerForecastId : record.data.generalLedgerForecastId,
							leafId : leafId,
							isAdmin : isAdmin
						},
						success : function (form, action) {
							Ext.getCmp('generalLedgerForecastCodeTemp').setValue(record.data.generalLedgerForecastDesc);
				...
				items : [{
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerForecastStore.each(function (record, fn, scope) {
									for (var access in generalLedgerForecastFlagArray) {
										record.set(generalLedgerForecastFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								generalLedgerForecastStore.each(function (record, fn, scope) {
									for (var access in generalLedgerForecastFlagArray) {
										record.set(generalLedgerForecastFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								...
								var sub_url = '';
								var modified = generalLedgerForecastStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&generalLedgerForecastId[]=' + modified[i].get('generalLedgerForecastId');
									if (isAdmin == 1) {
								...
								Ext.Ajax.request({
									...
									success : function (response, options) {
										...
										if (jsonResponse.success == true) {
											...
											generalLedgerForecastStore.reload();
	...
	var gridPanel = new Ext.Panel({
			...
			items : [generalLedgerForecastGrid]
		});
	// form entry
	
	var generalLedgerlChartOfAccountId = new Ext.ux.form.ComboBoxMatch({
        labelAlign: 'left',
        fieldLabel: stateIdLabel,
        name: 'generalLedgerlChartOfAccount',
        hiddenName: 'generalLedgerlChartOfAccount',
        valueField: 'generalLedgerlChartOfAccount',
        hiddenId: 'generalLedgerlChartOfAccount_fake',
        id: 'generalLedgerlChartOfAccount',
	...
	var generalLedgerForecastMonth = new Ext.form.numberField({
			labelAlign : 'left',
			fieldLabel : generalLedgerForecastMonthLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerForecastMonth',
			name : 'generalLedgerForecastMonth',
			id : 'generalLedgerForecastMonth',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
	...
	var generalLedgerForecastYear = new Ext.form.numberField({
			labelAlign : 'left',
			fieldLabel : generalLedgerForecastYearLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerForecastYear',
			name : 'generalLedgerForecastYear',
			id : 'generalLedgerForecastYear',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var generalLedgerForecastAmount = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : generalLedgerForecastAmountLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerForecastAmount',
			name : 'generalLedgerForecastAmount',
			id : 'generalLedgerForecastAmount',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var generalLedgerForecastDesc = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : generalLedgerForecastDescLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'generalLedgerForecastDesc',
			name : 'generalLedgerForecastDesc',
			id : 'generalLedgerForecastDesc',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var generalLedgerForecastId = new Ext.form.Hidden({
			name : 'generalLedgerForecastId',
			id : 'generalLedgerForecastId'
		}); // end form entry
	...
	var formPanel = new Ext.form.FormPanel({
			...
			items : [{
					xtype : 'fieldset',
					title : 'Form Entry',
					items : [generalLedgerForecastId,
						generalLedgerlChartOfAccountId,
						generalLedgerForecastTitle,
						generalLedgerForecastDesc,
						generalLedgerForecastReportType]
				}, {
					xtype : 'fieldset',
			...
			buttons : [{
					...
					iconCls : 'new',
					handler : function () {
						var id = Ext.getCmp('generalLedgerForecastId').getValue();
						var method = 'create';
						formPanel.getForm().submit({
							...
							success : function (form, action) {
								if (action.result.success == true) {
									...
									Ext.getCmp('generalLedgerForecastId').setValue(action.result.generalLedgerForecastId);
								} else {
									Ext.MessageBox.alert(systemErrorLabel, action.result.message);
								}
							},
							...
							fn : function (response) {
								if ('yes' == response) {
									Ext.Ajax.request({
										...
										params : {
											method : 'delete',
											generalLedgerForecastId : Ext.getCmp('generalLedgerForecastId').getValue(),
											leafId : leafId,
											isAdmin : isAdmin
										},
										success : function (response, options) {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerForecastId : Ext.getCmp('firstRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('firstRecord').getValue() >= 1) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerForecastId : Ext.getCmp('previousRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									generalLedgerForecastId : Ext.getCmp('nextRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												generalLedgerForecastId : Ext.getCmp('lastRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
						...
						if (Ext.getCmp('generalLedgerForecastId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								url : '../controller/generalLedgerForecastController.php',
								method : 'POST',
								waitTitle : systemLabel,
