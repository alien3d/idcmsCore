Ext.onReady(function () {
	...
	var voucherTypeFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : [ {
					type : 'string',
					dataIndex : 'voucherTypeSequence',
					column : 'voucherTypeSequence',
					table : 'voucherType',
					database : 'iFinancial'
				},{
					type : 'string',
					dataIndex : 'voucherTypeCode',
					column : 'voucherTypeCode',
					table : 'voucherType',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'voucherTypeDesc',
					column : 'voucherTypeDesc',
					table : 'voucherType',
					database : 'iFinancial'
				}, {
					type : 'list',
	...
	var voucherTypeColumnModel = [new Ext.grid.RowNumberer(),{
			dataIndex : 'voucherTypeCode',
			header : voucherTypeCodeLabel,
			sortable : true,
			hidden : false,
			width : 50
		},{
			dataIndex : 'voucherTypeSequence',
			header : voucherTypeSequenceLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
			dataIndex : 'voucherTypeDesc',
			header : voucherTypeDescLabel,
			sortable : true,
			hidden : false,
			width : 50
		},
	...
	var voucherTypeFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var voucherTypeGrid = new Ext.grid.GridPanel({
			...
			autoHeight : false,
			height : 400,
			columns : voucherTypeColumnModel,
			plugins : [voucherTypeFilters],
			selModel : new Ext.grid.RowSelectionModel({
				singleSelect : true
			}),
			viewConfig : {
			...
			listeners : {
				'rowclick' : function (object, rowIndex, e) {
					var record = voucherTypeStore.getAt(rowIndex);
					...
					formPanel.form.load({
						...
						params : {
							method : 'read',
							mode : 'update',
							voucherTypeId : record.data.voucherTypeId,
							leafId : leafId,
							isAdmin : isAdmin
						},
						success : function (form, action) {
							Ext.getCmp('voucherTypeCodeTemp').setValue(record.data.voucherTypeDesc);
				...
				items : [{
						...
						listeners : {
							'click' : function (button, e) {
								voucherTypeStore.each(function (record, fn, scope) {
									for (var access in voucherTypeFlagArray) {
										record.set(voucherTypeFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								voucherTypeStore.each(function (record, fn, scope) {
									for (var access in voucherTypeFlagArray) {
										record.set(voucherTypeFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								var url = '../controller/voucherTypeController.php?';
								var sub_url = '';
								var modified = voucherTypeStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&voucherTypeId[]=' + modified[i].get('voucherTypeId');
									if (isAdmin == 1) {
	...
	var gridPanel = new Ext.Panel({
			...
			items : [voucherTypeGrid]
		});
	// form entry
	
	var voucherTypeCode = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : voucherTypeCodeLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'voucherTypeCode',
			name : 'voucherTypeCode',
			id : 'voucherTypeCode',
			allowBlank : false,
	...
	var voucherTypeSequence = new Ext.form.NumberField({
			labelAlign : 'left',
			fieldLabel : voucherTypeSequenceLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'voucherTypeSequence',
			name : 'voucherTypeSequence',
			id : 'voucherTypeSequence',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var voucherTypeDesc = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : voucherTypeDescLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'voucherTypeDesc',
			name : 'voucherTypeDesc',
			id : 'voucherTypeDesc',
			allowBlank : false,
			blankText : blankTextLabel,
	...
	var voucherTypeId = new Ext.form.Hidden({
			name : 'voucherTypeId',
			id : 'voucherTypeId'
		}); // end form entry
	...
	var formPanel = new Ext.form.FormPanel({
			...
			buttons : [{
					...
					iconCls : 'new',
					handler : function () {
						var id = Ext.getCmp('voucherTypeId').getValue();
						var method = 'create';
						formPanel.getForm().submit({
							...
							success : function (form, action) {
								if (action.result.success == true) {
									...
									Ext.getCmp('voucherTypeId').setValue(action.result.voucherTypeId);
								} else {
									Ext.MessageBox.alert(systemErrorLabel, action.result.message);
								}
							},
							...
							fn : function (response) {
								if ('yes' == response) {
									Ext.Ajax.request({
										...
										params : {
											method : 'delete',
											voucherTypeId : Ext.getCmp('voucherTypeId').getValue(),
											leafId : leafId,
											isAdmin : isAdmin
										},
										success : function (response, options) {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												voucherTypeId : Ext.getCmp('firstRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('firstRecord').getValue() >= 1) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									voucherTypeId : Ext.getCmp('previousRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									voucherTypeId : Ext.getCmp('nextRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												voucherTypeId : Ext.getCmp('lastRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
						...
						if (Ext.getCmp('voucherTypeId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								url : '../controller/voucherTypeController.php',
								method : 'POST',
								waitTitle : systemLabel,
