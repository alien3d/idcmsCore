Ext.onReady(function () {
	...
	var staffByProxy = new Ext.data.HttpProxy({
			url : '../controller/lateInterestTypeController.php?',
			method : 'GET',
	...
	var lateInterestTypeProxy = new Ext.data.HttpProxy({
			url : '../controller/lateInterestTypeController.php',
			method : 'POST',
			success : function (response, options) {
				jsonResponse = Ext.decode(response.responseText);
	...
	var lateInterestTypeReader = new Ext.data.JsonReader({
			totalProperty : 'total',
			successProperty : 'success',
			messageProperty : 'message',
			idProperty : 'lateInterestTypeId'
		});
	var lateInterestTypeStore = new Ext.data.JsonStore({
			proxy : lateInterestTypeProxy,
			reader : lateInterestTypeReader,
			autoLoad : true,
			autoDestroy : true,
			pruneModifiedRecords : true,
			baseParams : {
			...
			root : 'data',
			fields : [{
					name : 'lateInterestTypeId',
					type : 'int'
				}, {
					name : 'lateInterestTypeSequence',
					type : 'string'
				}, {
					name : 'lateInterestTypeCode',
					type : 'string'
				}, {
					name : 'lateInterestTypeDesc',
					type : 'string'
				}, {
					name : 'isDefault',
					type : 'boolean'
	...
	var lateInterestTypeFilters = new Ext.ux.grid.GridFilters({
			encode : false,
			local : false,
			filters : [ {
					type : 'string',
					dataIndex : 'lateInterestTypeSequence',
					column : 'lateInterestTypeSequence',
					table : 'lateInterestType',
					database : 'iFinancial'
				},{
					type : 'string',
					dataIndex : 'lateInterestTypeCode',
					column : 'lateInterestTypeCode',
					table : 'lateInterestType',
					database : 'iFinancial'
				}, {
					type : 'string',
					dataIndex : 'lateInterestTypeDesc',
					column : 'lateInterestTypeDesc',
					table : 'lateInterestType',
					database : 'iFinancial'
				}, {
					type : 'list',
	...
	var lateInterestTypeColumnModel = [new Ext.grid.RowNumberer(),{
			dataIndex : 'lateInterestTypeCode',
			header : lateInterestTypeCodeLabel,
			sortable : true,
			hidden : false,
			width : 50
		},{
			dataIndex : 'lateInterestTypeSequence',
			header : lateInterestTypeSequenceLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
			dataIndex : 'lateInterestTypeDesc',
			header : lateInterestTypeDescLabel,
			sortable : true,
			hidden : false,
			width : 50
		}, {
	...
	var lateInterestTypeFlagArray = ['isDefault', 'isNew', 'isDraft', 'isUpdate', 'isDelete', 'isActive', 'isApproved', 'isReview', 'isPost'];
	var lateInterestTypeGrid = new Ext.grid.GridPanel({
			name : 'lateInterestTypeGrid',
			id : 'lateInterestTypeGrid',
			border : false,
			store : lateInterestTypeStore,
			autoHeight : false,
			height : 400,
			columns : lateInterestTypeColumnModel,
			plugins : [lateInterestTypeFilters],
			selModel : new Ext.grid.RowSelectionModel({
				singleSelect : true
			}),
			viewConfig : {
			...
			listeners : {
				'rowclick' : function (object, rowIndex, e) {
					var record = lateInterestTypeStore.getAt(rowIndex);
					...
					formPanel.form.load({
						...
						params : {
							method : 'read',
							mode : 'update',
							lateInterestTypeId : record.data.lateInterestTypeId,
							leafId : leafId,
							isAdmin : isAdmin
						},
						success : function (form, action) {
							Ext.getCmp('lateInterestTypeCodeTemp').setValue(record.data.lateInterestTypeDesc);
				...
				items : [{
						...
						listeners : {
							'click' : function (button, e) {
								lateInterestTypeStore.each(function (record, fn, scope) {
									for (var access in lateInterestTypeFlagArray) {
										record.set(lateInterestTypeFlagArray[access], true);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								lateInterestTypeStore.each(function (record, fn, scope) {
									for (var access in lateInterestTypeFlagArray) {
										record.set(lateInterestTypeFlagArray[access], false);
									}
								});
							}
						}
						...
						listeners : {
							'click' : function (button, e) {
								var url = '../controller/lateInterestTypeController.php?';
								var sub_url = '';
								var modified = lateInterestTypeStore.getModifiedRecords();
								for (var i = 0; i < modified.length; i++) {
									var dataChanges = modified[i].getChanges();
									sub_url = sub_url + '&lateInterestTypeId[]=' + modified[i].get('lateInterestTypeId');
									if (isAdmin == 1) {
	...
	var gridPanel = new Ext.Panel({
			...
			items : [lateInterestTypeGrid]
		});
	// form entry
	
	var lateInterestTypeCode = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : lateInterestTypeCodeLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'lateInterestTypeCode',
			name : 'lateInterestTypeCode',
			id : 'lateInterestTypeCode',
			allowBlank : false,
	...
	var lateInterestTypeSequence = new Ext.form.NumberField({
			labelAlign : 'left',
			fieldLabel : lateInterestTypeSequenceLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'lateInterestTypeSequence',
			name : 'lateInterestTypeSequence',
			id : 'lateInterestTypeSequence',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var lateInterestTypeDesc = new Ext.form.TextField({
			labelAlign : 'left',
			fieldLabel : lateInterestTypeDescLabel + '<span style=\'color: red;\'>*</span>',
			hiddenName : 'lateInterestTypeDesc',
			name : 'lateInterestTypeDesc',
			id : 'lateInterestTypeDesc',
			allowBlank : false,
			blankText : blankTextLabel,
			style : {
				textTransform : 'uppercase'
	...
	var lateInterestTypeId = new Ext.form.Hidden({
			name : 'lateInterestTypeId',
			id : 'lateInterestTypeId'
		}); // end form entry
	...
	var formPanel = new Ext.form.FormPanel({
			...
			items : [{
					xtype : 'fieldset',
					title : 'Form Entry',
					items : [lateInterestTypeId,
						lateInterestTypeCode,
						lateInterestTypeSequence,
						lateInterestTypeDesc]
				}, {
					xtype : 'fieldset',
			...
			buttons : [{
					...
					iconCls : 'new',
					handler : function () {
						var id = Ext.getCmp('lateInterestTypeId').getValue();
						var method = 'create';
						formPanel.getForm().submit({
							...
							success : function (form, action) {
								if (action.result.success == true) {
									...
									Ext.getCmp('lateInterestTypeId').setValue(action.result.lateInterestTypeId);
								} else {
									Ext.MessageBox.alert(systemErrorLabel, action.result.message);
								}
							},
							...
							fn : function (response) {
								if ('yes' == response) {
									Ext.Ajax.request({
										...
										params : {
											method : 'delete',
											lateInterestTypeId : Ext.getCmp('lateInterestTypeId').getValue(),
											leafId : leafId,
											isAdmin : isAdmin
										},
										success : function (response, options) {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												lateInterestTypeId : Ext.getCmp('firstRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('firstRecord').getValue() >= 1) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									lateInterestTypeId : Ext.getCmp('previousRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
						...
						if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								...
								params : {
									method : 'read',
									lateInterestTypeId : Ext.getCmp('nextRecord').getValue(),
									leafId : leafId,
									isAdmin : isAdmin
								},
								success : function (form, action) {
					...
					handler : function () {
								...
								success : function (response, options) {
									...
									if (jsonResponse.success == true) {
										...
										formPanel.form.load({
											...
											params : {
												method : 'read',
												lateInterestTypeId : Ext.getCmp('lastRecord').getValue(),
												leafId : leafId,
												isAdmin : isAdmin
											},
											success : function (form, action) {
						...
						if (Ext.getCmp('lateInterestTypeId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
							formPanel.form.load({
								url : '../controller/lateInterestTypeController.php',
								method : 'POST',
								waitTitle : systemLabel,
