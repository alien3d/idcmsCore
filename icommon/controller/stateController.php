 ...
 * @author hafizan
 * @package state
 * @subpackage statev1,v2,v3,v4,v5
...
class StateClass extends ConfigClass {
    ...
    public function read() {
        ...
        if ($this->isAdmin == 0) {
            if ($this->q->vendor == self::MYSQL) {
                $this->auditFilter = "	AND `state`.`isActive`		=	1	";
    ...
    function update() {
        ...
        if ($total == 0) {
            ...
            if ($this->getVendor() == self::MYSQL) {
                $sql = "
				UPDATE		`state`
				SET 		`stateName`		=	'" . $this->model->getStateName() . "',
            ...
            $this->q->tableName = $this->model->getTableName();
            $this->q->primaryKeyName = $this->model->getPrimaryKeyName();
            $this->q->primaryKeyValue = $this->model->getStateId(0, 'single');
            $this->q->audit = $this->audit;
            $this->q->update($sql);
            if ($this->q->execute == 'fail') {
                echo json_encode(array("success" => false, "message" => $this->q->responce));
    ...
    function duplicate() {
        ...
        if ($this->getVendor() == self::MYSQL) {
			...
			SELECT	`stateName`
			FROM 	`state`
			WHERE 	`stateName` 	= 	'" . $this->model->getStateName() . "'
			AND		`isActive`		=	1";
        } else if ($this->getVendor() == self::MSSQL) {
            $sql = "
			SELECT	[stateName]
        ...
        if ($total > 0) {
            ...
            echo json_encode(array("success" => true, "total" => $total, "message" => "Duplicate Record", "stateDesc" => $row ['stateDesc']));
            exit();
        } else {
