Ext.onReady(function() {
    ...
    var paymentProxy = new Ext.data.HttpProxy({
        url: '../controller/paymentController.php',
        method: 'POST',
        success: function(response, options) {
            jsonResponse = Ext.decode(response.responseText);
    ...
    var paymentFilters = new Ext.ux.grid.GridFilters({
        encode: encode,
        local: local,
        filters: [{
            type: 'string',
    ...
    var paymentColumnModel = [new Ext.grid.RowNumberer(), {
        id: 'action',
        header: 'Task',
        xtype: 'actioncolumn',
        width: 50,
        items: [{
            ...
            handler: function(grid, rowIndex, colIndex) {
                var record = paymentStore.getAt(rowIndex);
                ...
                formPanel.form.load({
                    ...
                    success: function(form, action) {
                        if (action.result.success == true) {
                            ...
                            Ext.getCmp('paymentDescTemp').setValue(record.data.paymentDesc);
    ...
    var paymentDescTemp = new Ext.form.Hidden({
        name: 'paymentDescTemp',
        id: 'paymentDescTemp'
    });
    var paymentDesc = new Ext.form.TextField({
        labelAlign: 'left',
        fieldLabel: paymentDescLabel + '<span style=\'color: red;\'>*</span>',
        hiddenName: 'paymentDesc',
        name: 'paymentDesc',
        id: 'paymentDesc',
        allowBlank: false,
        ...
        listeners: {
            blur: function() {
                if (Ext.getCmp('paymentDesc').getValue().length > 0) {
                    Ext.Ajax.request({
                        url: '../controller/paymentController.php',
                        method: 'GET',
                        params: {
                            method: 'duplicate',
                            leafId: leafId,
                            paymentDesc: Ext.getCmp('paymentDesc').getValue()
                        },
                        success: function(response, options) {
                            jsonResponse = Ext.decode(response.responseText);
                            if (jsonResponse.success == true) {
                                if (jsonResponse.total > 0) {
                                    if (Ext.getCmp('paymentDescTemp').getValue() != Ext.getCmp('paymentDesc').getValue()) {
                                        duplicate = 1;
                                        duplicateMessageLabel = duplicateMessageLabel + Ext.util.Format.uppercase(Ext.getCmp('paymentDesc').getValue()) + ':' + +Ext.util.Format.uppercase(jsonResponse.paymentDesc);
                                        Ext.MessageBox.alert(systemErrorLabel, duplicateMessageLabel);
                                        Ext.getCmp('paymentDesc').setValue('');
                                    }
                                }
                            } else {
                                Ext.MessageBox.alert(systemLabel, jsonResponse.message);
    ...
    var paymentId = new Ext.form.Hidden({
        name: 'paymentId',
        id: 'paymentId'
    }); // end form entry
    ...
    var formPanel = new Ext.form.FormPanel({
        ...
        items: [{
            xtype: 'fieldset',
            title: 'Form Entry',
            items: [paymentId, paymentDesc, paymentDescTemp]
        },
        {
            xtype: 'fieldset',
        ...
        },
        {
            ...
            iconCls: 'new',
            handler: function() {
                var id = Ext.getCmp('paymentId').getValue();
                var method = 'create';
                formPanel.getForm().submit({
                    ...
                    success: function(form, action) {
                        if (action.result.success == true) {
                            ...
                            Ext.getCmp('paymentId').setValue(action.result.paymentId);
                        } else {
                            Ext.MessageBox.alert(systemErrorLabel, action.result.message);
                        }
                    },
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        paymentId: Ext.getCmp('firstRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('firstRecord').getValue() >= 1) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            paymentId: Ext.getCmp('previousRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
            ...
            handler: function() {
                ...
                if (Ext.getCmp('nextRecord').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        ...
                        params: {
                            method: 'read',
                            paymentId: Ext.getCmp('nextRecord').getValue(),
                            leafId: leafId,
                            isAdmin: isAdmin
                        },
                        success: function(form, action) {
        ...
        },
        {
            ...
            handler: function() {
                        ...
                        success: function(response, options) {
                            ...
                            if (jsonResponse.success == true) {
                                ...
                                formPanel.form.load({
                                    ...
                                    params: {
                                        method: 'read',
                                        paymentId: Ext.getCmp('lastRecord').getValue(),
                                        leafId: leafId,
                                        isAdmin: isAdmin
                                    },
                                    success: function(form, action) {
                ...
                if (Ext.getCmp('paymentId').getValue() <= Ext.getCmp('lastRecord').getValue()) {
                    formPanel.form.load({
                        url: '../controller/paymentController.php',
                        method: 'POST',
                        waitTitle: systemLabel,
